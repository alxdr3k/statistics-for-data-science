...
between options or approve a non-trivial action, two parts govern the request.
The **authority gate** and **must-not-ask** categories below are always loaded —
they decide *whether* to ask. The request body schema and validation contract —
how the request is shaped, plus the hard-reject/soft-warning rules — live in
[`docs/operator-decision-request-schema.md`](docs/operator-decision-request-schema.md),
loaded on demand when you actually compose a request.

**This section plus that schema reference are the canonical runtime contract.**
`docs/adr/0003-operator-decision-request-enforcement.md` (ADR-0003) is the
source-repo design record present in repos that carry the boilerplate's
numbered-docs structure; universal-profile adopter repos (AGENTS.policy.md +
CLAUDE.md + the synced schema reference, no numbered docs) enforce the contract
from those files directly without needing ADR-0003. If they ever disagree with
ADR-0003, they win for runtime; ADR-0003 should be updated to match in the
source repo.
...
### Request body schema and enforcement (load before composing)

When the authority gate says to ask, you **MUST** read
[`docs/operator-decision-request-schema.md`](docs/operator-decision-request-schema.md)
and generate the request through the schema defined there **before drafting**.
A request not produced through that schema is **invalid** — treat it as a hard
reject. That reference is the single source of truth for the request body field
table, `options[]` cardinality, the word budget, the hard-reject vs
soft-warning rules, evidence verification, anti-patterns, and the 5-defense
enforcement stack. It is synced into adopter repos alongside this file, so it is
always available — do not draft from memory.
