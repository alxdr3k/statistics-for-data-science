...
---
<!-- my-skill:generated
skill: review-and-land
base-sha256: 1cbba3dfcecdbf9ef172928f021c015b6914f6e2d84563347163d0ec59496089
overlay-sha256: e3b0c44298fc1c149afbf4c8996fb92427ae41e4649b934ca495991b7852b855
output-sha256: 1cbba3dfcecdbf9ef172928f021c015b6914f6e2d84563347163d0ec59496089
do-not-edit: edit .codex/skill-overrides/review-and-land.md instead
-->

현재 작업 중인 PR에 대해 codex 리뷰를 기다리고, feedback 코멘트를 수정·push하고, 통과 신호를 받으면 정책에 맞게 PR을 land한다. review-and-land은 자체 review/land logic을 보유하지 않으며, 다음 두 building block을 위임 호출하는 **thin shim**이다:
...
Long wait silence rule:
- review-loop / `wait-codex-review.sh` 같은 poller가 의미 있는 새 stdout을 내지 않으면 사용자에게 진행 멘트를 쓰지 않는다.
- "아직 대기 중", "계속 polling 중" 같은 heartbeat성 응답은 금지다.
- 사용자에게 말하는 시점은 pass, feedback, terminal error, auth/permission 문제, PR head 변경, 사용자의 새 지시처럼 분기 판단에 필요한 사건이 생겼을 때뿐이다.
...
- review-and-land은 orchestrator-only다. worker context에서 호출하면 land 단계가 `LAND_FORBIDDEN=1`에 의해 거부된다 (PA-1.x worker guard + PA-2.2 land 거부의 3차 enforcement). worker는 `/review-loop`을 직접 호출해 pass 신호만 인계하고 후속 land는 orchestrator가 결정한다.
- review-and-land이 직접 실행하는 mutation은 (1) `scripts/land-pr.sh` 호출 (orchestrator land path), (2) `mcp__github__unsubscribe_pr_activity` (review-loop의 Path A subscription 정리)뿐이다. 모든 review wait/feedback/push는 review-loop이 담당한다.
...
cross-cwd 상황에서는 PR URL 또는 PR 번호+repo를 review-loop으로 전달한다. codex 리뷰 요청/대기는 GitHub PR repo를 기준으로 동작하고, feedback 수정이 필요할 때만 review-loop이 PR `headRepository/headRefName`으로 로컬 head-branch worktree를 해석해 그 worktree에서 commit/push한다.
...
1. PR이 draft가 아닌가? (`gh -R "$REVIEW_LOOP_PASS_REPO" pr view "$REVIEW_LOOP_PASS_PR_NUMBER" --json isDraft -q .isDraft`)
2. required checks가 모두 SUCCESS인가? 미완료/실패 항목이 있으면 `gh -R "$REVIEW_LOOP_PASS_REPO" pr checks "$REVIEW_LOOP_PASS_PR_NUMBER" --watch`로 완료를 기다린다. cross-repo PR에서 ambient repo로 잘못 게이트하지 않도록 land 호출과 같은 `repo`/`pr_number` (review-loop pass payload) 출처를 쓴다.
...
review-loop이 terminal payload를 emit하면 review-and-land은 추가 land 시도를 하지 않고 caller에게 상태를 인계한다.
...
terminal payload는 review-and-land이 그대로 caller-visible로 출력한다 (재포장 없음).

## 환경변수

review-loop과 land-pr이 사용하는 env var를 그대로 따른다. review-and-land 자체가 추가하는 env var는 없다.
...
`LAND_PR_EXPECTED_HEAD_SHA`는 review-and-land이 review-loop pass payload의 `head_sha`로 자동 설정한다. caller가 별도로 export할 필요 없다.

## 인자

- 인자 없음: 현재 cwd가 PR repo의 작업 branch일 때만 현재 branch의 PR 자동 감지.
- PR 번호 또는 URL: 첫 positional 인자로 전달. 그대로 review-loop / land-pr에 propagate. cross-cwd에서는 PR URL을 선호하고, PR 번호만 쓰면 repo(`owner/repo`, 예: `CODEX_REPO`)도 함께 명시한다.

`$ review-and-land` 또는 `/review-and-land`만으로 충분. 명시적 PR 번호를 받으면 `/review-and-land 47` 형식.
