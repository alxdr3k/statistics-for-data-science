...
<!-- my-skill:generated
skill: pingpong
base-sha256: f924613ad94d8a2d079ae1d91b28751736600a1eaac5d08faef2e71776e11c94
overlay-sha256: e3b0c44298fc1c149afbf4c8996fb92427ae41e4649b934ca495991b7852b855
output-sha256: f924613ad94d8a2d079ae1d91b28751736600a1eaac5d08faef2e71776e11c94
...
`<session_id>` 인자는 생략 가능하다 — Tier-1(단일 active)은 그대로 쓰고,
Tier-2(2+ active)는 read-only면 배너와 함께 last-touched, mutating이면 exact sid
또는 snapshot selector를 요구한다. 자세한 규칙은 아래 "Sticky session pointer" 섹션 참고.
...
## Sticky session pointer (대화 세션 단위, DEC-025 / DEC-053 · ADR-0005)

같은 agent 대화 안에서 한 번 session에 연결하면 이후 호출에서 `<session_id>`
인자를 생략할 수 있다. 런타임이 셸 env에 안정적인 대화 id를 주입하므로 (Claude
`CLAUDE_CODE_SESSION_ID`, Codex `CODEX_THREAD_ID` — 대화 내 안정·대화 간 고유·
`codex resume` 보존) helper가 그 id로 per-conversation pointer(active session
set + last_touched)를 직접 보관한다(INV-0005-1). env id가 없는 환경(예: `env -i`
subprocess)에서는 helper pointer가 no-op이 되고 agent의 conversation memory가
fallback이다(DEC-025) — 대체가 아니라 additive.

**`--agent <현재 agent>`는 agent 호출에서 필수다.** `resolve`/`snapshot`/`select`를
부를 때 자기 agent(`codex` 또는 `claude`)를 항상 명시한다. `--agent`가 없으면
런타임 id가 하나뿐인 셸에서만 그 id로 resolve되고, `CLAUDE_CODE_SESSION_ID`와
`CODEX_THREAD_ID`가 **둘 다** 있는 셸(companion)에서는 모호하다고 보고
`resolvable:false`가 된다(INV-0005-6 locality).

**단, both-runtime 셸에서는 helper pointer 자체가 비어 있다.** `init`/`write`의
pointer upsert는 message `--sender`가 아니라 **호출 대화의 env id**로 동작하는데,
둘 다 있으면 어느 agent pointer를 만질지 추측하지 않고 **no-op**한다(P5-F1).
따라서 그런 셸에서는 `--agent`를 줘도 `resolve`/`snapshot`/`select`가 빈
active set을 돌려줄 수 있다 — 이 환경에서는 helper pointer/selector에 의존하지
말고 conversation memory 또는 exact sid fallback을 유지한다. helper pointer +
ephemeral selector 경로는 **런타임 id가 하나뿐인 셸에서만** 신뢰할 수 있다.

### 활성 집합 조회 / 위생 (INV-0005-5)

- `resolve --agent <현재 agent> --json` → `{resolvable, count, ambiguous,
  conversation_id, active:[{session_id, topic, last_touched}]}`. 매 조회 전
  helper가 active set을 canonical `session.json.status`와 대조해 reconcile하고
  closed/abandoned를 prune한다. env id가 없거나 pointer가 unreliable(`.dirty` /
  lock busy)하면 `resolvable:false` — 이때 conversation-memory fallback으로 간다.
- pointer state의 가치는 **내구성 + Tier-1 회복**이지 모호성 제거가 아니다.
  stale entry가 false ambiguity나 authority처럼 보이는 last-touched cache를
  만들지 않게 한다.

### Set / Clear (helper 자동 + conversation-memory fallback, DEC-025)

helper는 `init`/`write` 성공 시 호출 대화의 pointer에 그 sid를 best-effort로
upsert하고 reconcile이 closed/abandoned를 자동 prune한다. 단 upsert는 env id가
없거나 둘 다 있는(both-runtime) 셸에서는 no-op이므로(위 참조), 그런 환경을 위해
agent도 conversation memory를 다음 시점에 갱신한다.

- **Set** — initiator는 `start`의 `init --json` 성공 직후 출력된 `session_id`를,
  reviewer는 `join <sid>`가 `status=open` + `reviewer_agent` 일치를 통과한 직후
  그 sid를 "현재 active"로 기억한다(첫 write까지 기다리지 않음).
- **Clear** — `stop`이 close decision(`next_action=close` AND
  `session_close=true`)을 land시킨 직후, 또는 `stop` case D / 직접 `abandon`이
  성공한 직후(`agent_dialog_abandon` ack 또는 `already:true`) 그 sid 기억을
  비운다. close-intent(`session_close=false`)만 land한 상태에서는 user의 명시
  stop이 뒤따른 직후에 비운다. abandoned/closed sid를 남겨두면 다음 호출이
  stale sid로 resolve된다.

### Resolve 규칙 — two-tier, operation-classed (INV-0005-2/3)

`<session_id>`를 **명시하면** 항상 그 값을 쓴다(conversation memory의 현재
active도 그 값으로 갱신). 아래는 인자를 **생략**했을 때만 적용한다. topic은
display/filter 전용이며 authority가 아니다. single-active는 강제하지 않는다 —
한 agent가 두 주제로 두 peer session을 spawn하는 "codex 1 : cc 2"
워크플로(DEC-034)는 합법이다.

1. **명시 인자 우선** — 사용자가 `<session_id>`를 주면 그대로 사용.
2. **Tier-1 (active 정확히 1개)** — read-only·mutating 모두 그 session을 쓴다.
3. **Tier-2 (active 2개 이상)** — 명령의 종류로 갈린다 (INV-0005-2 keystone:
   2+ active에서 mutating은 ambient resolve 금지):
   - **read-only** (`read` / `watch` / `transcript` / `whose-turn`) —
     last-touched로 default할 수 있으나 반드시 resolved-session 배너를 보인다:
     `resolved: <topic> (<sid 끝 4자리>)`. 사용자가 다른 session을 의도했으면
     명시 sid로 다시 부른다.
   - **mutating** (`continue`의 response/decision, `stop`, `note`, `relay`) —
     silent last-touched default를 **금지**한다. exact sid 또는 아래 validated
     ephemeral snapshot selector를 요구한다.
4. **active 없음** — 사용자에게 `<session_id>` 명시(또는 `/pingpong start`)를
   안내한다.

같은 conversation에서 새 `start`/`join`을 하면 새 sid가 추가로 누적된다(이전 sid를
덮지 않음). active 1개에서 새로 start/join 하면 2개가 되어 Tier-2가 적용된다.

### Ephemeral snapshot selector (Tier-2 mutating disambiguation, INV-0005-4/6)

Tier-2에서 mutating 명령의 sid가 생략되면 raw 번호를 live active-set에 바로
대응시키지 않는다 — list 출력과 사용자 입력 사이에 active-set이 바뀌면 그 번호가
다른 session을 가리키는 silent mis-route(index TOCTOU)가 나기 때문이다. 대신:

1. `snapshot --agent <현재 agent> --json` → `{resolvable, snapshot_id,
   ambiguous, entries:[{index, session_id, topic, last_touched}]}`. helper가 그
   대화의 active set을 reconcile한 뒤 `snapshot_id -> {index -> sid}` 바인딩을
   persist하고 번호 목록을 돌려준다(대화당 1개, 새 snapshot이 이전 것을 supersede).
2. 번호 목록을 사용자에게 보이고 번호(또는 exact sid)를 받는다.
3. 사용자가 번호 N을 고르면 `select <snapshot_id> N --agent <현재 agent>
   --json` → `{resolved:true, session_id, topic}`. helper는 **live set이 아니라
   그 snapshot에** 대해 resolve하고 consume-once + 만료 + 선택 sid still-active
   재검증을 적용한다. snapshot이 superseded / 만료 / 이미 consumed이거나 선택
   session이 더 이상 active가 아니면 `{resolved:false, reason}` — 그 사유를
   보이고 `snapshot`을 다시 돌린다. 사용자가 exact sid를 주면 selector 없이 그대로 쓴다.

selector는 `{agent, conversation_id, snapshot_id}`에 local하고 **non-portable**
하다(INV-0005-6). cross-agent handoff 텍스트·transcript는 bare 번호가 아니라
topic + sid 끝자리(또는 full sid)로 세션을 가리킨다 — 다른 대화/agent에서 받은
번호는 그 대화의 snapshot이 아니므로 helper가 reject한다.

durable alias namespace는 두지 않는다. B(multi-active disambiguation)는 위
ephemeral snapshot + op-classing + keystone 불변식으로 푼다. 짧은 별명 라우팅
도입 여부는 dogfood trigger까지 deferred다(Q-061; ADR-0005 Rejected alternatives).
...
명시 인자 우선, Tier-1 단일 active fallback. continue는 **mutating**이므로
Tier-2(2+ active)에서는 silent last-touched 없이 exact sid 또는 validated
snapshot selector를 요구한다(INV-0005-3).
...
- `<session_id>` 인자는 "Sticky session pointer" Resolve 규칙대로 해석. note는
  **mutating**이므로 Tier-2에서는 exact sid 또는 snapshot selector 요구 (INV-0005-3).
...
- **Passthrough 예외 (ADR-0004 INV-0004-3)**: relay도 delegation 적용 대상이
  ...
  agent가 채우는 것은 deterministic CLI 인자 매핑(`--messages` →
  `source_message_ids`, `--to` → `target_agent`)뿐이다.
- `<session_id>` 인자는 "Sticky session pointer" Resolve 규칙대로 해석. relay는
  **mutating**이므로 Tier-2에서는 exact sid 또는 snapshot selector 요구 (INV-0005-3).
...
- `<session_id>` 인자는 "Sticky session pointer" Resolve 규칙대로 해석. watch는
  **read-only**이므로 Tier-2(2+ active)에서는 last-touched로 default하되
  resolved-session 배너(`resolved: <topic> (<sid 끝 4자리>)`)를 보인다 (INV-0005-3).
...
- **per-turn step**: `pingpong-relay.sh step --session <sid> [--manual]
...
- **auto-relay는 기본 동작 (DEC-054/XAR-1Bb.6)**: bare `step`이 response 턴을
  자동수집·relay한다 (parallel은 synthesis + decision draft까지 emit). **수동
  모드는 `--manual`** (alias `--no-auto-relay`) — 매 턴 pause해 driver가 직접
  진행시킨다. `--auto-relay`는 backward-compat로 계속 허용되나 이제 no-op다.
- **capability 분리**: auto-relay(response 자동수집+relay; 기본 on, `--manual`로
...
`<session_id>` 인자는 위 "Sticky session pointer" Resolve 규칙대로 해석한다 —
명시 인자 우선, Tier-1 단일 active fallback. stop은 **mutating**이므로
Tier-2(2+ active)에서는 silent last-touched 없이 exact sid 또는 validated
snapshot selector를 요구한다(INV-0005-3). stop이 terminal close
(`session_close=true`) decision을 land시킨 직후 해당 sid의 conversation 기억을 비운다.
