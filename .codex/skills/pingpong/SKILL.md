...
<!-- my-skill:generated
skill: pingpong
base-sha256: 2740242932ca787a79f8224e3a70c5293f848e02571d167d76a5fe17410d58e3
overlay-sha256: e3b0c44298fc1c149afbf4c8996fb92427ae41e4649b934ca495991b7852b855
output-sha256: 2740242932ca787a79f8224e3a70c5293f848e02571d167d76a5fe17410d58e3
...
- 지원 sub-action: `start`, `join`, `continue`, `stop`, `note` (XAR-1A.2c.a),
  `relay` (XAR-1A.2c.b).
- 지원 message kind: `request`, `response`, `decision`, `note` (XAR-1A.2c.a),
  `relay` (XAR-1A.2c.b).
- full validation chain, full redaction pattern set, lock audit, SIGKILL
  대응은 후속 XAR-1A.2c slices에서 추가된다.
...
1. 한 agent에서 해당 표면에 맞게 `pingpong start "<topic 또는 첫 질문>"` 입력.
...
4, 5단계의 `<session_id>` 인자는 단일 active session에 한해 생략 가능 — 자세한
규칙은 아래 "Sticky session pointer" 섹션 참고.
...
Agent는 사용자 입력을 임의로 확장하지 않고 아래 공용 "Request body 결정적
규칙"을 그대로 적용한다.

#### Request body 결정적 규칙 (XAR-1A.3)

`start`와 `continue`의 다음 `request` 둘 다 같은 규칙으로 body를 만든다 —
LLM judgment 금지, 사용자 입력에서 기계적으로만 추출한다.

- `topic` = 사용자 입력의 **첫 줄** (`\n` 기준 분리 후 leading/trailing
  whitespace strip). 첫 줄이 80 chars를 넘으면 첫 80 chars + `"…"`로 자른다.
- `prompt`:
  - `start` (첫 request) 또는 pending note가 없는 `continue` 의 다음 request
    이면 `prompt` = 사용자 입력 **전체 그대로** (수정·요약·재배열·확장 없음).
  - `continue`의 다음 request 시점에 pending note가 있으면 (latest protocol
    message 이후 추가된 모든 `kind=note` message) 결정적으로 concat한다 —
    pending note 본문들을 message_id 순서대로 다음 형식으로 prepend:
    `[pending notes]\n- <note A body.text>\n- <note B body.text>\n...\n\n[next request]\n<사용자 입력 전체>` (대괄호 라벨과 줄바꿈은 literal). 그 외 수정·요약 없음. pending note가 없으면 wrapper 라벨을 붙이지 않는다 — 처음 두 케이스와 동일하게 사용자 입력만 그대로 쓴다. 이렇게 해야 새 request가 latest protocol message가 되어 reviewer-side note cutoff가 이동해도 그 note가 prompt에 보존된다 (round-4 codex P2).
- agent 본인의 부연 설명·추측·재구성은 body에 넣지 않는다. 사용자에게 보여
  주고 싶은 안내가 있으면 user-facing 문구로 분리한다.
- **선행 입력 검증**: 위 규칙으로 `topic`을 뽑은 결과가 빈 문자열이면 (사용자
  입력이 비어 있거나 첫 줄이 whitespace-only면) 사용자에게 "첫 줄에 topic 한
  줄을 적어 달라"고 안내하고 **helper 호출(`init`, `write`)을 시작하지
  않는다**. 이미 session을 만들고서 cleanup하는 흐름은 init-only orphan을 만들
  위험이 있으므로 입력 검증은 항상 helper 호출 전에 끝낸다.

#### start 호출 순서

1. 위 결정적 규칙으로 `topic`/`prompt`를 사용자 입력에서 build한다. 검증 실패
   (빈 topic 등)면 helper를 호출하지 말고 사용자에게 안내 후 종료.
2. `init --initiator <codex|claude> --topic "<topic>" --json` 호출. `init`의
   `--topic`은 위에서 build한 `topic`을 그대로 전달한다 (이 값이 helper
...
- 각 finding은 **`F` + 1-based 순번**으로 `finding_id`를 매긴다 (`F1`, `F2`,
  `F3`, ...) — XAR-1A.3 결정적 명명. helper는 `^[A-Za-z0-9._-]+$` 형식만
  강제하지만 skill 규칙으로 이 한 가지 패턴만 사용한다. 같은 round 안에서
  중복 finding_id는 허용되지 않는다 (helper도 reject). round가 바뀌면 다시
  `F1`부터 시작한다 — finding_id는 round-local이고, decision의 finding_id
  reference는 같은 round 안의 response에 매칭된다.
...
1. `read --session <sid> --json`으로 `topic`, `initiator_agent`,
   `reviewer_agent`, `status` 확인. status가 `open`이 아니면 stop.
...
3. 가장 최근 protocol message의 `kind`로 분기 (note는 latest 계산에서 제외 —
   ...
   - `response`: 현재 agent가 initiator라면 `decision`을 쓴다. body 결정적
     구성 (XAR-1A.3):
     - `decisions[]`: latest `response.findings[]`의 모든 `finding_id`를
       **빠짐 없이** 같은 finding_id로 dispose. `action` ∈
       `accepted|rejected|deferred`, `reason_one_line`은 한 줄. findings가
       비어 있으면 `decisions: []`.
   ...
   - `decision`이고 `next_action == continue`:
     - `next request text`가 주어졌으면 다음 `request`를 쓴다. body는
       위 `start` 섹션의 "Request body 결정적 규칙" 전체를 그대로 재사용한다.
       특히 `prompt`는 그 규칙의 pending-note concat 분기를 그대로 적용한다 —
       latest protocol message 이후 pending note(`kind=note`)가 있으면
       `[pending notes]` + `[next request]` wrapper로 결정적 concat, 없으면
       `next request text` 전체 verbatim. 빈 topic 검증도 동일하게 helper
       호출 전에 적용한다 (비어 있으면 helper를 호출하지 말고 사용자에게 다시
       prompt를 받는다).
     - 비어 있으면 사용자에게 "다음 prompt를 주거나 `/pingpong stop <sid>` 실행"
       안내.
   - `decision`이고 `next_action == needs_user`:
     - 사용자 결정이 들어온 뒤 `next request text`가 주어졌으면 다음 `request`를
       쓴다. body는 위 `continue` 분기와 동일하게 "Request body 결정적 규칙"을
       전부 재사용한다 — pending-note concat 분기 포함, 빈 topic이면 helper를
       호출하지 말고 사용자에게 다시 prompt를 받는다.
...
2. **case A — clean review**: latest **protocol** message가 `response`이고 `findings`가 비어 있다.
   - `decision` body를 `{"decisions":[], "next_action":"close", "session_close":true, "summary":"합의되어 종료"}`로 작성.
3. **case B — findings 있는 response**: latest **protocol** message가 `response`이고 `findings`가 비어 있지 않다.
   - 모든 `findings[].finding_id`에 대해 dispose를 채운다. `action`은 사용자
     ...
     사용자 입력 또는 짧은 자동 요약.
   - `decision` body를 `{"decisions":[{"finding_id":"F1","action":"deferred","reason_one_line":"close 시점"}, ...], "next_action":"close", "session_close":true, "summary":"사용자 판단으로 종료"}`로 작성.
4. **case C — latest protocol message = decision(continue 또는 needs_user)**: 새 close decision을
   작성한다.
   - `decision` body를 `{"decisions":[], "next_action":"close", "session_close":true, "summary":"후속 request 없이 종료"}`로 작성.
...
6. case A/B/C인 경우: `write --session <sid> --kind decision --sender <current_agent> --body-file <dec.json> --json`.
...
| `write` | `--session`, `--kind request|response|decision|note`, `--sender codex|claude|user`, `--body-file`, `[--json]` |
...
- role 불일치: `request.sender != initiator`, `response.sender != reviewer`,
  `decision.sender ∉ {initiator, user}`, `note.sender != user`.
