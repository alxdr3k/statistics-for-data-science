---
name: run
description: "단일 진입점 — run-registry로 active-run collision guard + register 후 전체 개발 사이클(sync -> discover -> implement -> verify -> review -> land)을 실행하고 exit 시 release. dev-cycle 흡수(DEC-049). 플래그: --loop [N], --phase <id>, --opus-audit-every <N>"
---
<!-- my-skill:generated
skill: run
base-sha256: 7b1c2e9ec8ac5d01dd293eca10e40baf50230e1b338387a18c55536fa53ad63a
overlay-sha256: e3b0c44298fc1c149afbf4c8996fb92427ae41e4649b934ca495991b7852b855
output-sha256: 7b1c2e9ec8ac5d01dd293eca10e40baf50230e1b338387a18c55536fa53ad63a
do-not-edit: edit .codex/skill-overrides/run.md instead
-->

# /run

> **codex 정본**: 이 파일은 `commands/run.md`(공통 정본)와 핵심 contract(진입
> 시퀀스 · Step · flag · helper 계약)를 동기화해야 한다 (DEC-049 F4). 한쪽을 바꾸면
> 양쪽을 함께 갱신한다. codex 환경에는 Opus sub-agent가 없어 `--opus-review`만 codex
> 정본에서 제외된다.

`$run`(또는 "Use run to ...")은 단일 에이전트 개발 사이클의 **단일 진입점**이다.
`scripts/run-registry.sh`로 현재 작업 세션을 register하고 (같은 logical repo의 두
번째 run을 collision으로 거부), 이어서 전체 개발 사이클(sync → discover → implement
→ verify → review → land)을 실행한 뒤 exit 시 release한다. dev-cycle 커맨드를 흡수한
결과다(DEC-049).
...
- 멀티 에이전트(`run-team`)는 본 skill 범위 밖이다 (PA-3.1). 첫 인자가 `--team` 또는
  `-team` literal flag form이면 즉시 안내 후 종료한다 (DEC-030 D6):
...
dev-cycle→run rename 이전의 in-flight run state(브리프 / `run.json` / loop marker)는
**`run-helper`가 startup에서 repo root 기준으로 `.run/`로 승계**하고 구 brief 파일명
(`dev-cycle-run-id` 등)을 `run-*`로 rename한다. **여기서 inline `mv`를 하지 않는다** —
wrapper가 먼저 mv하면 그 뒤 helper adopt가 `.run/` 존재를 보고 fail-closed skip해
파일명 migration이 누락되고 `validate-brief`/`finish-cycle-json`이 실패한다(P2-A).
adopt는 helper에 위임하고 wrapper는 collision/register만 담당한다. loop marker는 hook
dual-read(`.run` ‖ `.dev-cycle`)라 adopt 시점과 무관하게 인식된다.

### 2. collision reservation + register (DEC-049 F2/F3)

collision preflight/reservation을 canonical repo root에서 먼저 수행하고, register로
run을 잠근 다음 개발 사이클(worktree는 Step 4)로 들어간다. lock order는 **`registry`
→ `main_sync_holder`** 하나로 고정한다 — registry lock은 register reservation/update의
짧은 임계구역에서만 잡고, 개발 사이클 helper 실행 중에는 보유하지 않는다 (F3).
...
if [ "$(printf '%s' "$ACTIVE_JSON" | jq 'length')" -gt 0 ]; then
  ...
  echo "옵션: 기존 run 종료 대기 / 다른 worktree / stale이면 \"$RUN_REGISTRY\" gc --repo $REPO_ID"
  exit 6
fi
AGENT="${AGENT:-codex}"; case "$AGENT" in claude|codex|other) ;; *) AGENT=codex ;; esac
# register가 collision으로 exit 6을 낼 수 있다. status 확인 후 run_id 없으면 개발
# 사이클로 진입하지 않는다 (P2-C — guard 우회 방지).
if ! REG_JSON="$("$RUN_REGISTRY" register --type solo --workspace "$WS" --agent "$AGENT")"; then
  echo "ERROR: register 거부 (race/legacy collision) — 개발 사이클 진입 중단."
...
안에서 active scan을 다시 수행해 race면 exit 6을 반환한다 (PA-1.1c).

### 3. exit trap: run_id 기반 release (DEC-049 F2)

release는 cwd가 아니라 **run_id 기반**으로 idempotent하게 동작한다.

```bash
# 장기 workflow가 RUN_HEARTBEAT_TTL(기본 300s)을 넘겨도 registry entry가 살아있도록
# background heartbeat를 띄운다 — 다른 /run의 gc가 still-running entry를 abort하는 것을
# 막는다(DEC-049 #2). exit trap이 loop 정리 + release.
...
register가 끝나면 아래 Flags ~ Loop의 개발 사이클을 수행한다. collision guard는 origin
기준 logical-repo scope(`REPO_ID`)로 동작하므로 Step 4의 worktree 생성과 무관하게
정확하다.
...
- 검증 출력 위생: 검증·테스트 출력은 `tail`/`head`로 잘라 판독하지 않는다 — suite silent-abort, FAIL 라인, 실제 exit code를 가린다. 전체 출력을 파일/변수로 받아 green을 보고하기 전에 확인하고, exit code는 display 절단(파이프 끝 `tail`/`grep` 등)과 분리해 별도로 캡처한다. 신규 테스트는 단언이 실제로 효력을 갖는지(존재하지 않는 경로·no-op로 vacuously-true 통과가 아닌지) positive-control로 확인한다.
- 검증을 파이프로 연결하면 `$?`/`PIPESTATUS`가 검증기가 아니라 파이프 끝 명령(`tail`/`grep` 등)의 exit를 잡을 수 있다. 검증 성패로 분기하려면 **검증기의 실제 exit status**로 게이팅한다 — 출력을 변수로 받은 직후 exit를 캡처하고(`OUT="$(검증)"; RC=$?; [ "$RC" -eq 0 ] || { printf '%s\n' "$OUT"; exit 1; }`) 그 status로 분기한다. 출력 문구 검사(`grep passed` 등)는 진단용일 뿐 게이트가 아니다 — 검증기가 nonzero로 죽으면서 출력에 success 단어를 포함하면(예: `1 failed, 3 passed`) 문구 게이팅은 실패를 통과시킨다. FAIL이면 commit/amend하지 않는다.
...
- `CHANGE_SCOPE_JSON.review_inputs`의 base_range(committed `<base>...HEAD`)를 리뷰한다. 아래 Pre-review gate를 통과하면 worktree가 clean하므로 staged/unstaged/untracked input은 비어 있다 (잔존하면 게이트가 막는다).
...
| 1회차, `implementer=claude` | `/codex:adversarial-review --base "$REVIEW_BASE"` |
| 2~3회차 기본 | `/codex:adversarial-review --base "$REVIEW_BASE"` |
| 4회차~ 기본 | `/codex:review --base "$REVIEW_BASE"` |

`/codex:review`·`/codex:adversarial-review` 서브커맨드는 `--model` 플래그를 받지 않는다
(companion 1.0.4 기준 `--model`은 `task` 서브커맨드 전용). `adversarial-review`에 넘긴
positional 인자는 review focus text로 처리되고, `review`는 focus text가 있으면 거부한다
(throw). 따라서 `--model gpt-5.5` 같은 인자를 붙이면 모델 고정이 아니라 focus
오염(adversarial-review) 또는 리뷰 실패(review)가 된다. reviewer 모델은
codex 기본값/`~/.codex/config.toml`에서 결정하며, run은 인자로 고정하지 않는다.

리뷰어 호출 인자 표면(주의): (i) worktree 흐름에서 diff가 task worktree에 있으면 codex
review에 `--cwd <worktree>`를 명시해야 diff를 본다 — main 세션 cwd(clean)에서 돌리면 빈
diff를 리뷰한다. (ii) native `review`(defect pass)는 startup에 git object probe가 있어
history가 큰 repo에서 수십 초~분 느릴 수 있다(adversarial-review엔 없음); 진행 정체로
오인하지 말고 출력 파일 byte 증가·pid 생존으로 교차 확인한다. (iii) codex 호출 stdin은
단일 소스만 준다 — prompt를 `- < promptfile`로 줄 때 `</dev/null`을 함께 주면 빈
프롬프트로 헛돈다.
...
- small local-only는 `--opus-review`가 없을 때만, 그리고 나머지 reviewer row보다 먼저 판정한다.
  ...
  또한 **design-doc/spec 게이트(아래 Review Loop의 DEC-058 예외 (iv)) 통과 슬라이스는 small이어도 small-skip하지 않는다** — Step 6 design-doc acceptance를 반드시 거친다(authoritative 정의는 그 예외 절).
...
- 현재 surface에 Opus sub-agent가 없으면 `implementer=codex` round-1은
  `/codex:adversarial-review --base "$REVIEW_BASE"`로 fallback한다.
...
- 모든 repo는 같은 입력 규칙을 쓴다. 위 Pre-review gate를 통과하면 worktree는 clean하고 의도한 변경은 모두 committed HEAD에 있으므로, Review Pass 입력은 committed `$REVIEW_BASE...HEAD`다. reviewer가 committed HEAD를 리뷰하게 해 Step 9 PR review와 review target을 일치시킨다.
...
**예외 — design-doc/spec acceptance (DEC-058)**: 변경이 **dense 설계/spec 문서에 국한된 슬라이스**일 때만 적용한다. 게이트는 `change_scope.kind`가 아니라 **변경 경로**로 판정한다 (classifier가 `docs/design/**`·`docs/adr/**`를 파일명 패턴에 따라 `docs_only_low_risk`로도 분류하므로 kind anchor는 신뢰 불가 — 아래 전부 충족):
- (i) substantive 변경 파일이 **`docs/design/**` 또는 `docs/adr/**`** (dense·상호의존 설계/spec 문서)다.
- (ii) 그 외 변경은 **tracking-only** — `docs/08_DECISION_REGISTER.md`·`docs/07_QUESTIONS_REGISTER.md`·`docs/04_IMPLEMENTATION_PLAN.md`·`docs/context/current-state.md`·thin nav docs뿐. **tracking-only의 enforceable 경계**: 이 sidecar 편집은 status·lineage/evidence·register bookkeeping(DEC/Q row 추가·answered 전환 등)·links/nav 갱신**만** 허용된다. acceptance criteria·prerequisites·current contract/behavior·implementation requirements·validation requirements를 바꾸는 편집이 하나라도 있으면 그 슬라이스는 **design-doc이 아니며** 일반 0-actionable review를 받는다(IMPL_PLAN/current-state가 read order상 앞서 normative source가 될 수 있으므로, 그 normative 변경은 design-doc acceptance로 우회 불가). 설계/ADR이 Q ratify/close를 동반하는 bookkeeping은 허용.
- (iii) **executable agent contract를 하나도 바꾸지 않는다** — diff에 `commands/**`·`codex/skills/**`·`scripts/**`·`deploy.sh`·`tests/**`·`AGENTS*.md`·`CLAUDE.md`가 있으면 design-doc acceptance **불가**다. 이들은 실행 계약이라 강한 "0 actionable" 리뷰 대상이고, dense spec이 아니라 codex 리뷰가 수렴한다(`commands/run.md`·`codex/skills/run/SKILL.md`를 바꾸는 슬라이스는 design-doc 슬라이스가 **아니다**).
- (iv) **small-skip 제외**: 위 게이트를 통과한 design-doc/spec 슬라이스는 diff가 작거나 `docs_only_low_risk`로 분류돼도 **Step 6 small-skip을 적용하지 않는다** — 반드시 Step 6를 실행해 design-doc acceptance(architecture 반증·내부 모순 검사 포함)를 거친다(F-A3 — 안 그러면 순수 design-doc 변경이 small-skip으로 본 게이트를 우회한다). small-skip 판정(아래 Review Loop)보다 이 제외가 우선한다.

근거: codex defect-review는 dense·상호의존 *설계 spec*에서 per-diff 적대 리뷰가 항상 인접 consistency/precision edge를 하나 더 찾아 구조적으로 0-actionable에 **수렴하지 않는다**(실증 PA-3.1-design-v2 17 round; 이슈 #121). executable 계약 변경엔 이 문제가 없어 제외한다.

게이트 통과 슬라이스의 pass 조건 = **design-doc acceptance** (종료 mechanics는 아래 numbered loop step 3 design-doc 분기가 authoritative): (a) **architecture/계약 타당** — 리뷰가 더 이상 architecture 반증(high severity·approach-overturn)을 안 냄, (b) **문서 내부 모순 0** — 권위 섹션 간·summary↔권위 간 contradiction 없음(targeted grep/read 확인), (c) **미해결 정밀도 명시 impl-scope** — 정확한 crash/recovery 시퀀스·exact JSON shape 등은 구현 슬라이스 helper-test로 문서에 명시 deferral. (b) 내부 모순은 **ship-blocker** — land 전 반드시 해소("수렴 안 함"이 "모순 방치" 핑계 아님). small-skip과 달리 리뷰는 수행하고 *종료 판정*만 다르다.

각 pass는 다음 순서를 엄격히 따른다:

1. **리뷰어 실행** (Codex 또는 Opus) → finding 목록 수신
2. finding을 적대적/비판적으로 재평가 → 유효한 actionable finding 분류 + 각 finding의 severity 표시(architecture/high vs consistency/precision)
3. **종료 판정** — 슬라이스 종류로 분기:
   - **code 슬라이스 (기본)**: 유효 actionable finding == **0**이면 Step 7로 간다 (**Review Pass**). 0이 아니면 4로.
   - **design-doc/spec 슬라이스 (위 게이트 통과 시에만)**: 이번 pass에 (a) architecture 반증(high·approach-overturn) **0** AND (b) 문서 내부 모순 **0**이면 그 round를 **clean**으로 카운트. **2회 연속 clean**이고 잔여 precision finding이 모두 impl-scoped 항목(이슈/슬라이스)으로 **등록 완료**됐으면 Step 7로 간다 (**design-doc acceptance pass**). 조건 미충족이면 4로. architecture 반증·내부 모순이 남아 있으면 절대 clean이 아니다.
4. **유효 actionable finding 처리**: batch 수정(또는 precision은 impl-scope 등록) → targeted verify → **1로 돌아가 리뷰어를 반드시 재실행**. **문서·deferral 등록을 포함해 어떤 변경이라도 하면 design-doc 분기의 '연속 clean' 카운트를 0으로 reset**한다 — 수정 후엔 fresh review round를 다시 거쳐야 2-clean을 재축적한다(등록 자체도 리뷰 대상).
...
- 의도한 파일만 stage하고 commit한다 — Step 6를 실행한 cycle은 Pre-review gate에서 이미 commit/amend됐고 worktree가 clean하므로 추가 commit 없이 그대로 사용하고, small-skip cycle만 여기서 commit한다. 그런 다음 `RUN_WORK_BRANCH`를 push한다. 이어서 PR body를 `PR_BODY_FILE` 경로의 파일로 작성하고 (아래 'Test plan 섹션' 참고) `check-test-plan`을 통과한 뒤 GitHub app 또는 `gh pr create`로 **draft가 아닌 open PR**을 생성한다. 생성 결과 URL에서 PR 번호를 추출해 `PR_NUMBER`에 저장하고 Step 9에서 같은 변수를 사용한다.
...
- 검증을 통과하면 `review-and-land` 스킬을 같은 세션에서 PR 번호와 함께 실행한다 (예: `review-and-land "$PR_NUMBER"`).
- cwd는 그대로 task worktree 안이어도 안전하다. `review-and-land` shim은 review/feedback 단계는 cwd에서 처리하고, land 단계만 자체적으로 safe non-head cwd로 이동해 `scripts/land-pr.sh`를 호출한다 (PR #68에서 land cwd split 도입). 호출이 끝난 뒤 caller cwd는 서브셸 격리로 그대로 유지된다.
- `review-and-land`은 review feedback 처리, checks 확인, merge까지 완료해야 한다. 해당 PR이 merge되기 전에는 cycle을 마치거나 다음 loop로 넘어가지 않는다.
- merge 완료 후 Step 9 cleanup. `review-and-land`은 cwd를 보존하므로 호출 직후 cwd는 task worktree 안이다. 다음을 그 순서대로 수행한다.
