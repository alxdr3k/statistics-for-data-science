...
- **<a id="ref-19-가중치"></a>[가중치](#note-19-가중치)**: 입력의 중요도를 숫자로 표현하는 값
- **<a id="ref-19-경사하강법"></a>[경사하강법](#note-19-경사하강법)**: 가중치를 조금씩 고치는 방식
...
- **필수**: 입력-가중치-<a id="ref-19-편향"></a>[편향](#note-19-편향)-활성화-출력 흐름, 비선형 활성화가 필요한 이유, <a id="ref-19-순전파"></a>[순전파](#note-19-순전파)와 <a id="ref-19-역전파"></a>[역전파](#note-19-역전파)의 역할 차이
- **심화**: <a id="ref-19-기울기-소실"></a>[기울기 소실](#note-19-기울기-소실)의 원인
- **나중**: 다층 네트워크의 표현력 이론

## 이 차시에서 꼭 붙잡을 설명 방식

<a id="ref-19-은닉층"></a>[은닉층](#note-19-은닉층)이 필요한 이유는 단순한 직선 경계로 나눌 수 없는 패턴이 많기 때문이다. 은닉층은 원래 입력을 다른 표현으로 바꿔서, 출력층이 더 쉽게 판단할 수 있는 특징을 만든다.

## 핵심 이론

### 먼저 잡는 직관

- **단층 <a id="ref-19-퍼셉트론"></a>[퍼셉트론](#note-19-퍼셉트론)**: 입력에 가중치를 곱해 더한 값이 기준을 넘는지 보고 하나의 판단을 만든다.
- **<a id="ref-19-다층-퍼셉트론"></a>[다층 퍼셉트론](#note-19-다층-퍼셉트론)**: 여러 층을 쌓으면 단순 직선으로 나누기 어려운 패턴도 조합해 표현할 수 있다.
- **<a id="ref-19-활성화-함수"></a>[활성화 함수](#note-19-활성화-함수)**: 활성화 함수는 층 사이에 비선형성을 넣어 여러 층을 쌓는 의미를 만든다.
...
- <a id="note-19-경사하강법"></a>[경사하강법](#ref-19-경사하강법): 손실이 줄어드는 방향으로 가중치를 조금씩 바꾸는 방법. ([처음 설명된 차시](../12-parametric-models/README.md#2-경사하강법과-학습률))
- <a id="note-19-편향"></a>[편향](#ref-19-편향): 판단 기준을 이동시키는 상수.
- <a id="note-19-순전파"></a>[순전파](#ref-19-순전파): 입력에서 출력 방향으로 예측값을 계산하는 과정.
- <a id="note-19-역전파"></a>[역전파](#ref-19-역전파): 손실의 기울기를 뒤에서 앞으로 전달하는 과정.
- <a id="note-19-기울기-소실"></a>[기울기 소실](#ref-19-기울기-소실): 앞쪽 층으로 갈수록 학습 신호가 약해지는 현상.
- <a id="note-19-은닉층"></a>[은닉층](#ref-19-은닉층): 입력과 출력 사이에서 특징을 만드는 층.
- <a id="note-19-퍼셉트론"></a>[퍼셉트론](#ref-19-퍼셉트론): 입력에 가중치를 곱해 판단하는 기본 신경망 단위.
- <a id="note-19-다층-퍼셉트론"></a>[다층 퍼셉트론](#ref-19-다층-퍼셉트론): 입력층과 출력층 사이에 은닉층을 둔 신경망.
