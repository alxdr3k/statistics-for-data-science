...
| 용어 | 먼저 이렇게 이해하기 |
|---|---|
| [퍼셉트론](#1-단층-퍼셉트론) | 입력에 가중치를 곱해 판단하는 기본 신경망 단위 |
| 가중치 | 입력의 중요도를 나타내는 숫자 |
| 편향 | 판단 기준을 이동시키는 상수 |
| [활성화 함수](#3-활성화-함수) | 출력 신호를 비선형으로 바꾸는 함수 |
| 은닉층 | 입력과 출력 사이에서 특징을 만드는 층 |
| 순전파 | 입력에서 출력 방향으로 예측값을 계산하는 과정 |
| [역전파](#4-역전파와-기울기-소실) | 손실의 기울기를 뒤에서 앞으로 전달하는 과정 |
| [기울기 소실](#4-역전파와-기울기-소실) | 앞쪽 층으로 갈수록 학습 신호가 약해지는 현상 |

## 핵심 개념 링크

- [퍼셉트론](#1-단층-퍼셉트론): 입력에 가중치를 곱해 판단하는 기본 신경망 단위
- [다층 퍼셉트론](#2-다층-퍼셉트론): 입력층과 출력층 사이에 은닉층을 둔 신경망
- [활성화 함수](#3-활성화-함수): 출력 신호를 비선형으로 바꾸는 함수
- [역전파](#4-역전파와-기울기-소실): 손실의 기울기를 뒤에서 앞으로 전달하는 과정
- [기울기 소실](#4-역전파와-기울기-소실): 앞쪽 층으로 갈수록 학습 신호가 약해지는 현상
...
- **이전에서 이어지는 점**: 18차시까지 일반 모델 운영을 봤다면, 19차시는 신경망의 가장 작은 판단 단위로 들어간다.
- **다음으로 이어지는 점**: 20차시에서는 여러 레이어가 데이터 형태별로 어떤 역할을 하는지 본다.
