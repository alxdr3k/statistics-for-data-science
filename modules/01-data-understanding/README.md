...
**KDD(Knowledge Discovery in Database/Databases)**는 데이터베이스 안에서 유용한 지식을 발견하는 과정이다. 강의 표기는 `Database`이고, 일반 자료에서는 `Databases` 표기도 자주 쓴다. 일반적인 단계는 다음과 같다.
...
이 차시에서는 KDD, CRISP-DM, 데이터 형태, 변수 타입, 시각화를 한 번에 묶는다. 이 범위의 핵심은 “데이터가 무엇인가”를 추상적으로 외우는 것이 아니라, 실제 표에서 단위, 변수, 관측치, 변수 설명, 측정 단위를 분리해 읽는 것이다.
