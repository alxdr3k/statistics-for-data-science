...
- **표**: <a id="ref-01-데이터"></a>[데이터](#note-01-데이터)를 행과 열로 놓고 읽는 기본 형태
- **질문**: 분석은 답할 질문이 있어야 시작된다
- **<a id="ref-01-변수"></a>[변수](#note-01-변수)**: 관측 대상의 특징을 적은 칸
...
- 한 행은 무엇을 의미하는가?
- 한 열은 무엇을 측정하는가?
- 각 값은 어떤 단위와 <a id="ref-01-척도"></a>[척도](#note-01-척도)로 기록되었는가?
- 이 데이터는 어떤 분석 질문에 답할 수 있고, 어떤 질문에는 답할 수 없는가?
- <a id="ref-01-시각화"></a>[시각화](#note-01-시각화)는 무엇을 보여 주며, 무엇을 숨기는가?
...
이 자료는 “<a id="ref-01-정형-데이터"></a>[정형 데이터](#note-01-정형-데이터)는 표 형태다”처럼 결론만 외우게 하지 않는다. 항상 이유를 함께 본다.
...
중요한 점은 저장 형태에 따라 준비 과정이 달라진다는 것이다. <a id="ref-01-비정형-데이터"></a>[비정형 데이터](#note-01-비정형-데이터)도 임베딩, 라벨, 특징 추출을 거치면 정형 데이터처럼 모델에 입력될 수 있다. 반대로 정형 데이터라도 변수 정의가 부정확하면 분석 가치가 낮다.
...
- <a id="note-01-정형-데이터"></a>[정형 데이터](#ref-01-정형-데이터): 엑셀 표처럼 행과 열이 뚜렷한 데이터.
- <a id="note-01-비정형-데이터"></a>[비정형 데이터](#ref-01-비정형-데이터): 사진, 음성, 긴 글처럼 표로 바로 읽기 어려운 데이터.
