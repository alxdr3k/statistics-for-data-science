...
- 분석 단위가 질문과 맞지 않을 때 어떤 왜곡이 생기는지 설명할 수 있다.
- 숫자처럼 보이는 값도 수치형이 아닐 수 있음을 설명할 수 있다.
- 정형, 반정형, 비정형 데이터를 예로 구분할 수 있다.
- 변수의 척도에 따라 평균, 비율, 거리 계산 가능 여부를 판단할 수 있다.
...
분석 단위가 질문과 맞지 않으면 통계량도 다른 뜻이 된다. 거래 100건의 평균 구매액은 “거래 한 번의 평균”이고, 고객 100명의 평균 구매액은 “고객 한 명의 평균”이다. 두 값은 모두 평균이지만 분모가 다르다. 초심자는 평균 공식을 보기 전에 “무엇을 몇 개로 나누는가?”를 먼저 말로 풀어야 한다.
...
척도는 뒤 차시의 통계 계산을 제한한다. 명목 척도는 개수와 비율을 셀 수 있지만 평균은 의미가 없다. 순서 척도는 순위 비교가 가능하지만 1점과 2점의 차이가 4점과 5점의 차이와 같다고 단정하기 어렵다. 등간 척도는 차이를 말할 수 있지만 0이 “없음”을 뜻하지 않을 수 있다. 비율 척도는 0이 실제 없음이므로 두 배, 절반 같은 비율 해석이 가능하다.

| 척도 | 가능한 대표 계산 | 조심할 계산 |
|---|---|---|
| 명목 | 빈도, 비율, 최빈값 | 평균, 표준편차, 거리 |
| 순서 | 중앙값, 분위수, 순위 비교 | 간격이 같다는 전제의 평균 |
| 등간 | 평균, 차이 | 두 배, 절반 같은 비율 |
| 비율 | 평균, 차이, 비율, 성장률 | 단위가 섞인 직접 비교 |

모델링에서도 같은 문제가 생긴다. 고객 ID를 숫자 입력으로 넣으면 모델은 ID의 크기와 차이를 신호로 오해할 수 있다. 그래서 식별자는 보통 제거하거나 조인 키로만 쓰고, 지역이나 상품군처럼 의미 있는 범주는 인코딩해서 쓴다.
