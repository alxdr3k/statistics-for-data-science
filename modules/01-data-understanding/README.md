...
|---|---|
| 데이터 | 관찰하거나 기록한 사실 |
| 데이터셋 | 분석하려고 모아 둔 데이터 묶음 |
| 행(row) | 한 사람, 한 거래, 한 상품처럼 관찰 대상 하나 |
| 열(column) | 나이, 가격, 날짜처럼 관찰 대상의 속성 하나 |
| 변수(variable) | 열과 비슷한 말. 분석에서 쓰는 속성 |
| 관측치(observation) | 보통 표의 한 행. 한 관찰 대상에 대해 기록된 값들의 묶음 |
| 관측값(value) | 특정 행과 열이 만나는 한 칸에 적힌 값 |
| 정형 데이터 | 엑셀 표처럼 행과 열이 뚜렷한 데이터 |
| 비정형 데이터 | 사진, 음성, 긴 글처럼 표로 바로 읽기 어려운 데이터 |
| [시각화](#8-시각화는-변수-타입과-질문의-함수다) | 숫자를 그래프나 그림으로 바꿔 보는 일 |

## 핵심 개념 링크

- [변수](#4-단위-변수-관측치): 관측 대상의 특징을 적어 둔 열
- [정량적 데이터](#3-정량적-데이터와-정성적-데이터): 숫자 크기와 차이를 계산할 수 있는 데이터
- [척도](#5-변수의-역할과-척도): 값을 어떤 규칙과 수준으로 측정했는지 나타내는 기준
- [시각화](#8-시각화는-변수-타입과-질문의-함수다): 숫자를 그래프나 그림으로 바꿔 보는 일
...
| 용어 | 이름의 뉘앙스 |
|---|---|
| Data | `given`, 즉 주어진 기록이라는 느낌이다. 분석가는 주어진 기록이 무엇을 뜻하는지 먼저 확인한다. |
| Dataset | 데이터를 분석할 수 있도록 한 묶음으로 모아 둔 것이다. |
| Structured Data | 구조가 잡혀 있다는 뜻이다. 행과 열이 있어 표처럼 읽을 수 있다. |
| Unstructured Data | 정해진 표 구조가 없다는 뜻이다. 이미지, 음성, 긴 글은 먼저 특징을 뽑아야 한다. |
| Variable | 변할 수 있는 값이라는 뜻이다. 사람마다 나이, 구매액, 지역이 달라지므로 변수라고 부른다. |
| Observation | 관찰한 기록이라는 뜻이다. 보통 표의 한 행이 하나의 관찰 대상이다. |
| KDD | PDF에는 `Knowledge Discovery in Database`로 적혀 있지만, 일반적으로는 `Knowledge Discovery in Databases`라고도 쓴다. 데이터베이스 안에서 쓸모 있는 지식을 찾아낸다는 이름이다. |
| CRISP-DM | 여러 산업에서 쓰기 위한 데이터 마이닝 표준 절차라는 이름이다. 업무 문제에서 시작하는 프로젝트 흐름에 가깝다. |
...
- **이전에서 이어지는 점**: 학습 방법에서 본 “읽고 확인하고 회고하기”를 실제 데이터 표 읽기로 옮긴다.
- **다음으로 이어지는 점**: 2차시에서는 이렇게 읽은 표에서 빈 값과 튀는 값을 어떻게 볼지 다룬다.
...
처음에는 약어를 외우려고 하지 않아도 된다. 둘 다 “데이터 분석 프로젝트가 보통 어떤 순서로 진행되는지”를 정리한 지도라고 보면 된다.

**KDD(Knowledge Discovery in Database/Databases)**는 데이터베이스 안에서 유용한 지식을 발견하는 과정이다. PDF 표기는 `Database`이고, 일반 자료에서는 `Databases` 표기도 자주 쓴다. 일반적인 단계는 다음과 같다.
...
중요한 점은 저장 형태에 따라 준비 과정이 달라진다는 것이다. 비정형 데이터도 임베딩, 라벨, 특징 추출을 거치면 정형 데이터처럼 모델에 입력될 수 있다. 반대로 정형 데이터라도 변수 정의가 부정확하면 분석 가치가 낮다.
