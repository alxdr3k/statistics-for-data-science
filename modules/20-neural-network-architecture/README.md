...
| [Embedding](#1-dense와-embedding) | 범주를 연속 벡터로 바꾸는 층 |
| Convolution | 작은 필터로 지역 패턴을 찾는 연산 |
| [RNN](#3-rnn-lstm-gru) | 이전 상태를 다음 계산에 사용하는 순환 신경망 |
| LSTM | 장기 정보를 보존하기 위한 게이트 구조 RNN |
| GRU | LSTM보다 단순한 게이트 구조 RNN |
| [Dropout](#4-보조-레이어) | 학습 중 일부 뉴런을 무작위로 제외하는 기법 |
| Pooling | 영역별 대표값으로 정보를 압축하는 연산 |

## 핵심 개념 링크

- [Dense](#1-dense와-embedding): 이전 층의 모든 입력이 현재 층의 모든 뉴런과 연결되는 층
- [Embedding](#1-dense와-embedding): 범주를 연속 벡터로 바꾸는 층
- [CNN](#2-cnn): 필터로 지역 패턴을 찾는 합성곱 신경망
- [RNN](#3-rnn-lstm-gru): 이전 상태를 다음 계산에 사용하는 순환 신경망
- [Dropout](#4-보조-레이어): 학습 중 일부 뉴런을 무작위로 제외하는 기법
...
| 용어 | 이름의 뉘앙스 |
|---|---|
| [Dense](#1-dense와-embedding) | 모든 입력과 뉴런이 빽빽하게 연결된 층이다. |
| [Embedding](#1-dense와-embedding) | 범주를 의미 있는 연속 벡터 공간에 심는다는 뜻이다. |
| Convolution | 작은 필터를 밀어 가며 지역 패턴을 찾는 연산이다. |
| Recurrent | 순환한다는 뜻이다. 이전 상태가 다음 계산에 들어간다. |
| [Dropout](#4-보조-레이어) | 일부 뉴런을 잠시 떨어뜨려 의존을 줄인다. |
...
- **이전에서 이어지는 점**: 19차시에서 뉴런 하나와 역전파를 봤다면, 20차시는 여러 레이어를 데이터 형태에 맞게 조립한다.
- **다음으로 이어지는 점**: 21차시에서는 이 레이어들의 가중치를 더 안정적으로 학습시키는 최적화 방법을 본다.
