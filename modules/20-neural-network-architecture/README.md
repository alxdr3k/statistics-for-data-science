...
### 5. 원본 강의자료 보강: 레이어별 입력과 출력

원본 PDF는 Dense, Embedding, CNN, RNN, LSTM, GRU, Dropout, Pooling을 각각의 역할로 구분한다. Dense는 모든 입력과 모든 뉴런을 연결하므로 파라미터 수가 빠르게 늘어난다. Embedding은 범주 ID를 의미 있는 연속 벡터로 바꾸며, 라벨 인코딩처럼 숫자 순서를 의미한다고 가정하지 않는다.
...
RNN 계열은 시퀀스 모델링 유형을 입력과 출력의 길이로 구분한다. many-to-one은 문장 분류처럼 여러 입력에서 하나의 출력을 만들고, one-to-many는 하나의 입력에서 여러 출력을 만든다. many-to-many는 번역이나 시계열 예측처럼 입력과 출력이 모두 순서 구조를 가진다. LSTM과 GRU는 게이트로 장기 의존성을 보존하려는 구조다.
