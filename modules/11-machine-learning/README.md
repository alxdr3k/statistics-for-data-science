...
데이터 분리는 역할을 섞지 않는 것이 핵심이다.

| 데이터 | 역할 | 섞으면 생기는 문제 |
|---|---|---|
| 훈련 데이터 | 모델이 규칙을 배우는 자료 | 성능 평가에 쓰면 외운 정도를 좋게 착각한다. |
| 검증 데이터 | 모델 선택과 튜닝에 쓰는 자료 | 반복해서 보면 검증 데이터에도 맞춰질 수 있다. |
| 테스트 데이터 | 마지막 일반화 성능 확인 | 튜닝에 쓰면 최종 평가 의미가 사라진다. |
...
지표는 “무엇을 틀리면 더 비싼가?”로 고른다. 암 진단처럼 실제 양성을 놓치는 비용이 크면 재현율을 더 중요하게 본다. 스팸 필터처럼 정상 메일을 스팸으로 보내는 비용이 크면 정밀도를 더 중요하게 볼 수 있다. 정확도는 클래스 비율이 균형일 때는 직관적이지만, 양성이 매우 드문 문제에서는 대부분을 음성으로 찍어도 높게 나올 수 있다.
...
데이터 누수는 실제 예측 시점에는 알 수 없는 정보가 학습에 들어가는 문제다. 예를 들어 고객 이탈을 예측하면서 이탈 후 발생한 환불 기록을 입력 변수로 넣으면 모델은 미래 정보를 본 셈이다. 이런 경우 검증 성능은 높아 보이지만 실제 배포에서는 무너질 수 있다.
