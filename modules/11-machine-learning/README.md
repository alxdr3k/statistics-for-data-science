...
### 5. 원본 강의자료 보강: 모델 선택과 일반화 오차

원본 PDF는 머신러닝 모델을 모수적 모델과 비모수적 모델로 나눈다. 모수적 모델은 정해진 형태와 제한된 파라미터 안에서 학습하고, 비모수적 모델은 데이터가 많아질수록 더 유연한 구조를 만들 수 있다. 모수적 모델은 빠르고 해석이 쉬운 경우가 많지만 형태 가정이 틀리면 과소적합될 수 있고, 비모수적 모델은 유연하지만 데이터와 계산 비용이 더 필요하다.
