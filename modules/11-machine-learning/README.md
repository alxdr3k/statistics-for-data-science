...
| [지도학습](#1-학습-유형) | 정답 라벨이 있는 데이터로 예측 규칙을 배우는 학습 |
| [비지도학습](#1-학습-유형) | 정답 없이 데이터 구조를 찾는 학습 |
| 강화학습 | 행동과 보상으로 좋은 전략을 배우는 학습 |
| 일반화 | 새 데이터에서도 잘 맞는 성질 |
| [성능 지표](#2-성능-지표) | 모델 결과를 평가하는 숫자 |
| [교차검증](#3-검증-방식) | 데이터를 여러 번 나누어 검증하는 방식 |
...
- [성능 지표](#2-성능-지표): 모델 결과를 평가하는 숫자
- [교차검증](#3-검증-방식): 데이터를 여러 번 나누어 검증하는 방식
- [편향-분산](#4-편향-분산): 모델이 너무 단순한 오류와 너무 흔들리는 오류의 균형
...
- **이전에서 이어지는 점**: 10차시까지 관계를 설명했다면, 11차시는 그 관계를 학습해 새 데이터에 적용하는 관점으로 바뀐다.
- **다음으로 이어지는 점**: 12차시에서는 정해진 식과 파라미터를 가진 모델들을 먼저 본다.
...
분류는 정확도, 정밀도, 재현율, F1, AUC를 쓴다. 회귀는 MAE, MSE/RMSE, R2, MAPE를 쓴다. 군집화는 실루엣 같은 구조 지표를 본다.
