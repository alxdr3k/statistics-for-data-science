...
- 트리의 깊이와 KNN의 k를 편향-분산 관점으로 읽는다.
...
비모수적 모델은 “모수가 없다”는 뜻이 아니라, 고정된 작은 수의 파라미터만으로 전체 형태를 정하지 않는다는 뜻에 가깝다. 데이터가 많아지면 트리 구조나 이웃 관계가 더 세밀해질 수 있다. 그래서 유연하지만, 검증 없이 복잡도를 키우면 과대적합되기 쉽다.
...
트리 깊이는 편향-분산 균형과 연결된다. 얕은 트리는 규칙이 단순해 놓치는 패턴이 많을 수 있으므로 편향이 크다. 깊은 트리는 훈련 데이터의 작은 예외까지 따라가므로 분산이 커질 수 있다. 가지치기는 이 균형을 조절하는 방법이다.
...
KNN에서 거리는 변수 스케일의 영향을 직접 받는다. 소득이 원 단위이고 나이가 년 단위이면 소득 차이가 거리 대부분을 차지할 수 있다. 그래서 KNN을 쓰기 전에는 표준화나 정규화를 통해 각 변수가 거리 계산에서 비슷한 영향력을 갖게 하는지 확인해야 한다.
