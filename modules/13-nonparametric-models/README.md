...
| 용어 | 먼저 이렇게 이해하기 |
|---|---|
| [의사결정나무](#1-의사결정나무-구조) | 질문을 따라 가지를 내려가며 예측하는 트리 모델 |
| 노드 | 분기 조건이나 결과가 놓이는 지점 |
| 가지 | 조건에 따라 다음 노드로 이어지는 연결 |
| 리프 | 최종 예측이 놓이는 마지막 노드 |
| 불순도 | 한 노드 안에 클래스가 얼마나 섞여 있는지 |
| [가지치기](#3-가지치기와-과적합) | 너무 복잡한 트리를 줄이는 과정 |
| [KNN](#4-knn) | 가까운 k개 이웃의 정보를 이용하는 모델 |
| [차원의 저주](#4-knn) | 차원이 늘수록 거리의 구분력이 약해지는 현상 |

## 핵심 개념 링크

- [의사결정나무](#1-의사결정나무-구조): 질문을 따라 가지를 내려가며 예측하는 트리 모델
- [분기 기준](#2-분기-기준): 데이터를 나눌 때 섞임이나 오차를 줄이는 질문을 고르는 기준
- [가지치기](#3-가지치기와-과적합): 너무 복잡한 트리를 줄이는 과정
- [KNN](#4-knn): 가까운 k개 이웃의 정보를 이용하는 모델
- [차원의 저주](#4-knn): 차원이 늘수록 거리의 구분력이 약해지는 현상
...
- **이전에서 이어지는 점**: 12차시가 식의 파라미터를 학습했다면, 13차시는 데이터의 분기와 이웃을 직접 활용한다.
- **다음으로 이어지는 점**: 14차시에서는 거리와 경계 개념을 더 정교하게 써서 마진을 최대화한다.
...
차원의 저주는 변수가 많아질수록 거리의 의미가 흐려지는 현상이다. 차원이 높으면 대부분의 점이 서로 멀게 느껴져 “가까운 이웃”을 고르는 기준이 약해질 수 있다. 그래서 KNN은 스케일링과 차원 선택이 특히 중요하다.
