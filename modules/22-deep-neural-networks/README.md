...
- **<a id="ref-22-역전파"></a>[역전파](#note-22-역전파)**: 깊은 층까지 학습 신호가 전달되는 원리
- **일반화**: 깊은 모델의 과적합과 <a id="ref-22-전이학습"></a>[전이학습](#note-22-전이학습) 판단 기준
...
- **필수**: 깊은 층이 표현을 단계적으로 만든다는 점, BatchNorm과 <a id="ref-22-잔차"></a>[잔차](#note-22-잔차) 연결이 학습 안정화를 돕는 이유, <a id="ref-22-word2vec"></a>[Word2Vec](#note-22-word2vec)과 전이학습의 큰 그림
- **심화**: <a id="ref-22-resnet"></a>[ResNet](#note-22-resnet)의 잔차 학습 직관
- **나중**: 대규모 사전학습 모델의 미세조정 전략
...
- **대표 <a id="ref-22-cnn-구조"></a>[CNN 구조](#note-22-cnn-구조)**: 깊은 CNN은 낮은 수준의 선과 색에서 시작해 점점 더 복잡한 패턴을 조합한다.
- **<a id="ref-22-batch-normalization"></a>[Batch Normalization](#note-22-batch-normalization)**: 층마다 입력 <a id="ref-22-분포"></a>[분포](#note-22-분포)가 크게 흔들리지 않도록 조정해 학습을 안정적으로 만든다.
- **Word2Vec과 표현 학습**: 단어의 의미를 주변 단어와의 관계 속에서 벡터 공간의 위치로 학습한다.
- **사전학습과 전이학습**: 큰 데이터에서 배운 표현을 새 문제에 가져와 적은 데이터에서도 시작점을 좋게 만든다.

### 1. 대표 CNN 구조

<a id="ref-22-alexnet"></a>[AlexNet](#note-22-alexnet)은 깊은 <a id="ref-22-cnn"></a>[CNN](#note-22-cnn)의 가능성을 크게 보였고, <a id="ref-22-vgg"></a>[VGG](#note-22-vgg)는 작은 필터를 깊게 쌓는 단순한 구조를 강조했다. GoogLeNet/<a id="ref-22-inception"></a>[Inception](#note-22-inception)은 여러 필터 크기를 병렬로 쓰고, ResNet은 잔차 연결로 매우 깊은 모델을 학습하기 쉽게 했다.
...
층 입력의 분포를 안정화해 학습을 돕는다. 내부 분포 변화가 너무 크면 각 층이 계속 새 입력 환경에 적응해야 하므로 학습이 불안정해진다.
...
큰 데이터에서 먼저 배운 표현을 새 문제에 가져오면 적은 데이터에서도 시작점이 좋아진다. 이것이 <a id="ref-22-transfer-learning"></a>[Transfer Learning](#note-22-transfer-learning), 즉 전이학습이다. 일부 층은 고정하고 일부 층만 미세조정할 수 있다.
...
2. Batch Normalization이 훈련과 추론에서 다른 통계를 쓴다는 점을 확인한다.
...
1. <a id="ref-22-심층-신경망"></a>[심층 신경망](#note-22-심층-신경망)은 여러 은닉층을 통해 신호를 점점 더 추상적인 표현으로 바꾼다.
...
- <a id="note-22-역전파"></a>[역전파](#ref-22-역전파): 손실의 기울기를 뒤에서 앞으로 전달하는 과정. ([처음 설명된 차시](../19-perceptron/README.md#4-역전파와-기울기-소실))
- <a id="note-22-전이학습"></a>[전이학습](#ref-22-전이학습): 큰 데이터에서 배운 표현을 새 문제에 가져와 활용하는 방법.
- <a id="note-22-잔차"></a>[잔차](#ref-22-잔차): 실제값과 예측값의 차이. ([처음 설명된 차시](../09-linear-regression/README.md#2-주요-가정))
- <a id="note-22-word2vec"></a>[Word2Vec](#ref-22-word2vec): 단어의 주변 문맥을 이용해 단어를 벡터로 학습하는 방법.
- <a id="note-22-resnet"></a>[ResNet](#ref-22-resnet): 잔차 연결로 매우 깊은 네트워크 학습을 돕는 구조.
- <a id="note-22-cnn-구조"></a>[CNN 구조](#ref-22-cnn-구조): 합성곱 층을 깊게 쌓아 이미지 특징을 단계적으로 배우는 구조.
...
- <a id="note-22-alexnet"></a>[AlexNet](#ref-22-alexnet): 깊은 CNN 성능을 크게 알린 대표 구조.
- <a id="note-22-cnn"></a>[CNN](#ref-22-cnn): 필터로 지역 패턴을 찾는 합성곱 신경망. ([처음 설명된 차시](../20-neural-network-architecture/README.md#2-cnn))
- <a id="note-22-vgg"></a>[VGG](#ref-22-vgg): 작은 합성곱 필터를 깊게 쌓은 CNN 구조.
- <a id="note-22-inception"></a>[Inception](#ref-22-inception): 여러 크기의 필터를 한 모듈에서 병렬로 쓰는 구조. 이름: 여러 크기의 필터를 한 모듈에서 함께 쓰는 구조로 알려져 있다.
- <a id="note-22-transfer-learning"></a>[Transfer Learning](#ref-22-transfer-learning): 이미 학습한 표현을 새 문제에 가져와 쓰는 방법.
- <a id="note-22-심층-신경망"></a>[심층 신경망](#ref-22-심층-신경망): 은닉층이 여러 개인 신경망.
