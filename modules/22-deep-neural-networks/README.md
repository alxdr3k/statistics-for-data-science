...
| 용어 | 먼저 이렇게 이해하기 |
|---|---|
| 심층 신경망 | 은닉층이 여러 개인 신경망 |
| Black box | 판단 근거를 사람이 직접 해석하기 어려운 모델 성질 |
| AlexNet | 깊은 CNN 성능을 크게 알린 대표 구조 |
| VGG | 작은 합성곱 필터를 깊게 쌓은 CNN 구조 |
| Inception | 여러 크기의 필터를 한 모듈에서 병렬로 쓰는 구조 |
| [ResNet](#1-대표-cnn-구조) | 잔차 연결로 매우 깊은 네트워크 학습을 돕는 구조 |
| [Batch Normalization](#2-batch-normalization) | 층 입력 분포를 안정화하는 기법 |
| Transfer Learning | 이미 학습한 표현을 새 문제에 가져와 쓰는 방법 |

## 핵심 개념 링크

- [CNN 구조](#1-대표-cnn-구조): 합성곱 층을 깊게 쌓아 이미지 특징을 단계적으로 배우는 구조
- [Batch Normalization](#2-batch-normalization): 층 입력 분포를 안정화하는 기법
- [Word2Vec](#3-word2vec과-표현-학습): 단어의 주변 문맥을 이용해 단어를 벡터로 학습하는 방법
- [전이학습](#4-사전학습과-전이학습): 큰 데이터에서 배운 표현을 새 문제에 가져와 활용하는 방법
- [ResNet](#1-대표-cnn-구조): 잔차 연결로 매우 깊은 네트워크 학습을 돕는 구조
...
| 용어 | 이름의 뉘앙스 |
|---|---|
| Deep | 층이 깊다는 뜻이다. 여러 단계의 표현 변환을 거친다. |
| Black box | 안에서 어떤 이유로 판단했는지 보기 어렵다는 뜻이다. |
| Inception | 여러 크기의 필터를 한 모듈에서 함께 쓰는 구조로 알려져 있다. |
| Residual | 잔차를 더해 깊은 네트워크의 학습을 돕는다는 뜻이다. |
| Fine Tuning | 이미 배운 모델을 새 문제에 맞게 미세하게 조정한다는 뜻이다. |
...
- **이전에서 이어지는 점**: 21차시에서 학습을 안정화하는 최적화 방법을 봤다면, 22차시는 더 깊은 구조를 실제로 학습시키는 아이디어를 본다.
- **다음으로 이어지는 점**: 이후에는 프로젝트별로 모델 구조, 데이터 규모, 계산 비용을 함께 판단하는 단계로 이어진다.
