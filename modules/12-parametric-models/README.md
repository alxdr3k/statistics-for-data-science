...
| [경사하강법](#2-경사하강법과-학습률) | 손실이 줄어드는 방향으로 가중치를 조금씩 바꾸는 방법 |
| 학습률 | 한 번에 움직이는 보폭 |
| 규제 | 모델이 너무 복잡해지지 않게 벌점을 주는 방법 |
| [L1 규제](#3-l1과-l2-규제) | 가중치 절댓값 합에 벌점을 주는 규제 |
| [L2 규제](#3-l1과-l2-규제) | 가중치 제곱합에 벌점을 주는 규제 |
| [나이브 베이즈](#4-나이브-베이즈) | 입력 변수 독립을 가정해 베이즈 정리로 분류하는 모델 |
| 평활화 | 확률이 0이 되거나 작은 표본이 과하게 흔들리는 문제를 줄이는 보정 |

## 핵심 개념 링크

- [가중치 기반 모델](#1-가중치-기반-모델): 입력마다 중요도인 가중치를 곱해 출력을 만드는 모델
- [경사하강법](#2-경사하강법과-학습률): 손실이 줄어드는 방향으로 가중치를 조금씩 바꾸는 방법
- [L1 규제](#3-l1과-l2-규제): 가중치 절댓값 합에 벌점을 주는 규제
- [L2 규제](#3-l1과-l2-규제): 가중치 제곱합에 벌점을 주는 규제
- [나이브 베이즈](#4-나이브-베이즈): 입력 변수 독립을 가정해 베이즈 정리로 분류하는 모델
...
- **이전에서 이어지는 점**: 11차시에서 머신러닝 큰 그림을 봤다면, 12차시는 정해진 식의 파라미터를 학습하는 모델부터 본다.
- **다음으로 이어지는 점**: 13차시에서는 정해진 식보다 데이터 구조를 더 직접 사용하는 비모수적 모델을 본다.
