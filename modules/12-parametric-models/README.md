...
- **<a id="ref-12-경사하강법"></a>[경사하강법](#note-12-경사하강법)**: 손실을 줄이는 방향으로 움직이는 감각
- **확률**: <a id="ref-12-나이브-베이즈"></a>[나이브 베이즈](#note-12-나이브-베이즈)의 사후확률 계산 언어
...
- **필수**: <a id="ref-12-가중치-기반-모델"></a>[가중치 기반 모델](#note-12-가중치-기반-모델)의 입력-가중치-출력 흐름, <a id="ref-12-학습률"></a>[학습률](#note-12-학습률)이 너무 크거나 작을 때 문제, L1/<a id="ref-12-l2-규제"></a>[L2 규제](#note-12-l2-규제) 차이
- **심화**: 나이브 베이즈의 조건부 독립 가정
- **나중**: Elastic Net과 확률 모델 비교

## 이 차시에서 꼭 붙잡을 설명 방식

<a id="ref-12-규제"></a>[규제](#note-12-규제)는 성능을 일부러 방해하는 것처럼 보이지만, 실제 목적은 훈련 데이터에 너무 딱 맞는 복잡한 <a id="ref-12-가중치"></a>[가중치](#note-12-가중치)를 막는 것이다. 작은 손실만 쫓으면 새 데이터에서 흔들릴 수 있으므로, 가중치 크기에도 비용을 붙여 더 단순한 규칙을 선호하게 만든다.
...
<a id="ref-12-선형회귀"></a>[선형회귀](#note-12-선형회귀), 로지스틱 회귀, 신경망은 입력과 가중치의 조합으로 출력을 만든다. 학습은 손실을 줄이는 가중치를 찾는 과정이다.
...
- <a id="note-12-경사하강법"></a>[경사하강법](#ref-12-경사하강법): 손실이 줄어드는 방향으로 가중치를 조금씩 바꾸는 방법.
- <a id="note-12-나이브-베이즈"></a>[나이브 베이즈](#ref-12-나이브-베이즈): 입력 변수 독립을 가정해 베이즈 정리로 분류하는 모델.
- <a id="note-12-가중치-기반-모델"></a>[가중치 기반 모델](#ref-12-가중치-기반-모델): 입력마다 중요도인 가중치를 곱해 출력을 만드는 모델.
- <a id="note-12-학습률"></a>[학습률](#ref-12-학습률): 한 번에 움직이는 보폭.
- <a id="note-12-l2-규제"></a>[L2 규제](#ref-12-l2-규제): 가중치 제곱합에 벌점을 주는 규제.
- <a id="note-12-규제"></a>[규제](#ref-12-규제): 모델이 너무 복잡해지지 않게 벌점을 주는 방법.
- <a id="note-12-가중치"></a>[가중치](#ref-12-가중치): 입력 변수의 영향력을 나타내는 숫자.
- <a id="note-12-선형회귀"></a>[선형회귀](#ref-12-선형회귀): 입력과 출력의 평균적 관계를 직선식으로 설명하는 모델. ([처음 설명된 차시](../09-linear-regression/README.md#1-단순회귀와-다중회귀))
