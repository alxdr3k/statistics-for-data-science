...
### 5. 원본 강의자료 보강: Hard/Soft Voting과 Gradient Boosting

원본 PDF는 보팅을 hard voting과 soft voting으로 나눈다. Hard voting은 각 모델이 고른 클래스에 투표해 다수결로 정한다. Soft voting은 각 모델이 낸 클래스 확률을 평균한 뒤 가장 높은 확률의 클래스를 고른다. 확률이 잘 보정된 모델들이라면 soft voting이 더 많은 정보를 쓸 수 있다.
