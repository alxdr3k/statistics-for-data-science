...
| 용어 | 먼저 이렇게 이해하기 |
|---|---|
| [확률변수](#1-확률변수와-분포) | 우연한 결과를 숫자로 바꾼 변수 |
| [확률분포](#1-확률변수와-분포) | 가능한 값과 그 값이 나올 가능성을 연결한 것 |
| 이산 확률변수 | 셀 수 있는 값을 갖는 확률변수 |
| 연속 확률변수 | 길이, 시간처럼 연속적인 값을 갖는 확률변수 |
| 기대값 | 오래 반복했을 때의 평균적인 결과 |
| 분산 | 기대값 주변에서 얼마나 퍼지는지 |
| 모수 | 분포의 모양을 정하는 숫자 |
| [중심극한정리](#4-기대값-분산-중심극한정리) | 일정 조건에서 표본평균이 표본 수가 커질수록 정규분포에 가까워진다는 원리 |

## 핵심 개념 링크

- [확률변수](#1-확률변수와-분포): 우연한 결과를 숫자로 바꾼 변수
- [분포](#1-확률변수와-분포): 값들이 어떤 모양으로 흩어져 있는지 나타내는 구조
- [확률분포](#1-확률변수와-분포): 가능한 값과 그 값이 나올 가능성을 연결한 것
- [이산 분포](#2-이산-분포와-연속-분포): 셀 수 있는 값마다 확률이 붙는 분포
- [연속 분포](#2-이산-분포와-연속-분포): 구간 아래 면적으로 확률을 읽는 분포
- [중심극한정리](#4-기대값-분산-중심극한정리): 일정 조건에서 표본평균이 표본 수가 커질수록 정규분포에 가까워진다는 원리
...
- **이전에서 이어지는 점**: 4차시의 확률 언어를 값들의 모양인 분포로 확장한다.
- **다음으로 이어지는 점**: 6차시에서는 이 분포를 기준으로 관측 결과가 드문지 판단한다.
