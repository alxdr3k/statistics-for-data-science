...
베르누이, 이항, 포아송처럼 개수를 세는 분포는 이산 분포다. 각 값마다 확률을 직접 붙일 수 있다. 정규, 지수, 감마처럼 연속적인 값을 다루는 분포는 곡선 아래 면적으로 확률을 읽는다. 그래서 PDF에서는 확률질량함수와 확률밀도함수를 구분한다.
...
![중심극한정리의 직관](assets/central-limit-146.png)
...
일정 시간 안의 사건 수를 세므로 포아송 분포를 먼저 떠올릴 수 있다. 핵심 모수는 평균 발생 횟수인 lambda다.
