...
주사위를 던지면 결과는 눈의 개수다. 이 결과를 X라는 숫자 <a id="ref-05-변수"></a>[변수](#note-05-변수)로 적으면 <a id="ref-05-확률변수"></a>[확률변수](#note-05-확률변수)가 된다. <a id="ref-05-확률분포"></a>[확률분포](#note-05-확률분포)는 X가 1, 2, 3 같은 값을 가질 가능성을 정리한 표나 곡선이다. 분석에서는 데이터 한 번의 결과보다 결과들이 반복될 때 어떤 모양을 만드는지가 더 중요하다.
...
<a id="ref-05-기대값"></a>[기대값](#note-05-기대값)은 오래 반복했을 때의 평균 위치이고, <a id="ref-05-분산"></a>[분산](#note-05-분산)은 그 주변의 퍼짐이다. <a id="ref-05-중심극한정리"></a>[중심극한정리](#note-05-중심극한정리)는 표본이 충분히 독립적이고 분산이 지나치게 이상하지 않은 조건에서, 원래 데이터가 꼭 정규분포가 아니어도 표본평균의 분포가 표본 수가 커질수록 정규분포에 가까워진다는 말이다. 이 원리 때문에 뒤에서 신뢰구간과 가설검정이 가능해진다.
