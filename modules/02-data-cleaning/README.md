...
- **행과 열**: <a id="ref-02-결측치"></a>[결측치](#note-02-결측치)와 <a id="ref-02-이상치"></a>[이상치](#note-02-이상치)가 어디에 생겼는지 찾는 기준
- **<a id="ref-02-변수"></a>[변수](#note-02-변수) 타입**: 수치형/범주형에 따라 정제 방법이 달라진다
- **<a id="ref-02-분포"></a>[분포](#note-02-분포) 모양**: 튀는 값이 오류인지 신호인지 판단하는 단서

## 개념 지도

<a id="ref-02-데이터-정제"></a>[데이터 정제](#note-02-데이터-정제)는 모델링 전에 데이터를 “예쁘게” 만드는 작업이 아니다. 분석 질문에 맞게 데이터의 오류, 결측, 이상치를 해석하고, 정보 손실과 <a id="ref-02-편향"></a>[편향](#note-02-편향)을 최소화하는 의사결정 과정이다.
...
- **필수**: 결측치가 생긴 이유 구분, 이상치가 오류인지 신호인지 판단, 삭제/<a id="ref-02-대치"></a>[대치](#note-02-대치)/표시 중 선택
- **<a id="ref-02-심화"></a>[심화](#note-02-심화)**: <a id="ref-02-lof"></a>[LOF](#note-02-lof)와 <a id="ref-02-isolation-forest"></a>[Isolation Forest](#note-02-isolation-forest)의 직관
- **나중**: 모델 기반 이상치 점수를 운영 규칙으로 연결

## 이 차시에서 꼭 붙잡을 설명 방식

정제에서는 “삭제한다”, “대치한다” 같은 처리 이름보다 **왜 그 처리가 위험할 수 있는지**가 더 중요하다.

예를 들어 평균 대치가 위험한 이유는 다음과 같다.

1. 평균 대치는 비어 있는 여러 칸을 모두 같은 값으로 채운다.
2. 같은 값이 반복해서 들어가면 데이터가 원래보다 덜 퍼져 보인다.
3. 그래서 분산이 작아지고, 변수 사이의 관계도 약해질 수 있다.
...
정제에서 가장 위험한 태도는 “문제가 있는 행을 지우면 깨끗해진다”는 생각이다. 결측과 이상치는 데이터 생성 과정의 흔적일 수 있다. 따라서 정제는 삭제 기술이 아니라 원인 추론과 편향 관리에 가깝다.
...
<a id="ref-02-선형-보간"></a>[선형 보간](#note-02-선형-보간)은 두 점 `(x0, y0)`, `(x1, y1)` 사이의 값을 다음처럼 채운다.
...
IQR은 중앙 50% 범위를 기준으로 하므로 극단값에 비교적 강하다. 다만 분포가 원래 비대칭이거나 꼬리가 긴 경우 정상값도 이상치 후보로 표시될 수 있다.
...
여기서 `h(x)`는 점 `x`가 고립되기까지의 평균 경로 길이이고, `c(m)`은 표본 크기 `m`에서의 기대 경로 길이다.
...
1. 결측 처리는 <a id="ref-02-결측-원인"></a>[결측 원인](#note-02-결측-원인)을 먼저 판단한 뒤 선택해야 한다.
...
- <a id="note-02-결측치"></a>[결측치](#ref-02-결측치): 비어 있는 값.
...
- <a id="note-02-데이터-정제"></a>[데이터 정제](#ref-02-데이터-정제): 분석 전에 이상한 값, 빈 값, 구조 문제를 점검하는 일.
- <a id="note-02-편향"></a>[편향](#ref-02-편향): 결과가 한쪽으로 치우치는 것.
- <a id="note-02-대치"></a>[대치](#ref-02-대치): 빈 값을 다른 값으로 채우는 일.
- <a id="note-02-심화"></a>[심화](#ref-02-심화): 첫 학습 때 완벽히 몰라도 되는 내용.
- <a id="note-02-lof"></a>[LOF](#ref-02-lof): 주변보다 밀도가 낮은 점을 이상치로 보는 방법. 이름: Local Outlier Factor다. 전체가 아니라 내 주변 지역의 밀도를 기준으로 이상함을 본다는 이름이다.
- <a id="note-02-isolation-forest"></a>[Isolation Forest](#ref-02-isolation-forest): 점을 빨리 혼자 떨어뜨리는 무작위 나무들을 이용한 이상치 탐지. 이름: 점을 고립시키는 나무를 많이 만든 숲이라는 뜻이다. 빨리 혼자 떨어지는 점을 이상치 후보로 본다.
- <a id="note-02-선형-보간"></a>[선형 보간](#ref-02-선형-보간): 앞뒤 값 사이를 직선으로 이어 빈 값을 채우는 방법.
...
- <a id="note-02-결측-원인"></a>[결측 원인](#ref-02-결측-원인): 값이 왜 비었는지에 대한 이유.
