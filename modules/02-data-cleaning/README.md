...
| 용어 | 먼저 이렇게 이해하기 |
|---|---|
| 데이터 정제 | 분석 전에 이상한 값, 빈 값, 구조 문제를 점검하는 일 |
| [결측치](#2-결측-메커니즘) | 비어 있는 값 |
| 결측 원인 | 값이 왜 비었는지에 대한 이유 |
| 대치 | 빈 값을 다른 값으로 채우는 일 |
| [이상치](#4-이상치의-의미) | 전체 흐름에서 유난히 튀는 값 |
| 편향 | 결과가 한쪽으로 치우치는 것 |
| 분포 | 값들이 어떤 모양으로 퍼져 있는지 |
| 잔차 | 실제 값과 예상 값의 차이 |
| 심화 | 첫 학습 때 완벽히 몰라도 되는 내용 |

## 핵심 개념 링크

- [결측치](#2-결측-메커니즘): 비어 있는 값
- [선형 보간](#3-결측-처리-방법): 앞뒤 값 사이를 직선으로 이어 빈 값을 채우는 방법
- [이상치](#4-이상치의-의미): 전체 흐름에서 유난히 튀는 값
- [LOF](#7-모델-기반-이상치-탐지): 주변보다 밀도가 낮은 점을 이상치로 보는 방법
- [Isolation Forest](#7-모델-기반-이상치-탐지): 점을 빨리 혼자 떨어뜨리는 무작위 나무들을 이용한 이상치 탐지

## 용어 이름 먼저 풀기

| 용어 | 이름의 뉘앙스 |
|---|---|
| Missing Value | 값이 없다는 뜻이다. 단순한 빈칸이 아니라 왜 비었는지를 함께 봐야 한다. |
| Imputation | 빠진 값을 대신 넣는다는 뜻이다. 값을 채우는 동시에 가정을 넣는 일이다. |
| Outlier | 바깥쪽에 있는 값이라는 뜻이다. 전체 흐름에서 떨어져 보이는 값을 말한다. |
| IQR | `Inter Quantile Range`다. 분위수 사이, 특히 Q1과 Q3 사이의 범위를 본다. |
| Z-score | 평균에서 표준편차 몇 개만큼 떨어졌는지를 점수처럼 표현한 값이다. |
| [LOF](#7-모델-기반-이상치-탐지) | `Local Outlier Factor`다. 전체가 아니라 내 주변 지역의 밀도를 기준으로 이상함을 본다는 이름이다. |
| [Isolation Forest](#7-모델-기반-이상치-탐지) | 점을 고립시키는 나무를 많이 만든 숲이라는 뜻이다. 빨리 혼자 떨어지는 점을 이상치 후보로 본다. |
...
- **이전에서 이어지는 점**: 1차시에서 표의 행과 열을 읽었다면, 이제 그 표가 믿을 만한지 점검한다.
- **다음으로 이어지는 점**: 3차시에서는 정제된 값을 분석하기 쉬운 형태로 바꾸는 변환을 다룬다.
...
| [선형 보간](#3-결측-처리-방법) | 앞뒤 순서의 값 사이를 직선으로 채운다. | 시간이나 순서가 있고 변화가 완만할 때 | 급격한 변화나 비선형 패턴을 놓침 |
...
위 그림의 왼쪽은 박스 플롯으로 단변량 이상치를 보는 예이고, 오른쪽은 산점도로 두 변수의 관계에서 벗어난 점을 보는 예다. 예를 들어 수확량만 보면 정상 범위에 있어도, 결실율 대비 수확량이 비정상적으로 낮다면 관계 기반 이상치일 수 있다. 이때는 수확량의 Z-score보다 “결실율로 예측한 수확량과 실제 수확량의 잔차”가 더 적절한 기준이다.
...
LOF는 전체적으로는 정상 범위처럼 보여도, 특정 지역 구조 안에서 고립된 점을 찾는 데 강하다. 반면 `k` 값과 거리 척도에 민감하다.
...
해설은 [answers.md](answers.md)에 있다. 먼저 직접 말로 답한 뒤 확인한다.
