...
- 데이터 정제가 단순히 “빈 값과 튀는 값을 없애는 작업”이 아니라 데이터의 정합성과 분석 신뢰도를 높이는 과정임을 설명한다.
- 결측을 MCAR, MAR, MNAR로 구분하고, 각 경우에 삭제가 만드는 편향을 판단한다.
- 제거, 결측 여부 보존, 단순 대치, 회귀 대치, 다중 대치, Hot deck, 선형 보간의 차이를 설명한다.
- 이상치를 비자연적 이상치와 자연적 이상치로 나누고, 오류와 신호를 구분한다.
- IQR, Z-score, 시각화, LOF, Isolation Forest가 이상치를 판단하는 기준을 비교한다.
- 이상치 탐지와 신규성 탐지의 차이를 설명한다.
- Tidy Data의 3원칙과 Long/Wide 변환이 왜 필요한지 설명한다.
...
MNAR은 가장 어렵다. 결측된 값 자체가 결측의 원인이므로 데이터 안의 다른 변수만으로는 편향을 완전히 제거하기 어렵다. 이 경우 민감도 분석, 별도 조사, 결측 여부 자체의 모델링이 필요할 수 있다.
