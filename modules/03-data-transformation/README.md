...
| 용어 | 먼저 이렇게 이해하기 |
|---|---|
| 변환(Transformation) | 데이터를 다른 형태로 바꾸는 일 |
| 정규화(Normalization) | 값의 스케일을 공통 기준에 맞추는 일 |
| 표준화(Standardization) | 평균을 0, 표준편차를 1 기준으로 맞추는 일 |
| 인코딩(Encoding) | 문자나 범주를 숫자 코드로 바꾸는 일 |
| 원-핫 인코딩 | 한 범주만 1이고 나머지는 0인 열을 만드는 방식 |
| 타깃 인코딩 | 범주를 목표 변수의 평균이나 비율로 바꾸는 방식 |
| 데이터 누수(Data Leakage) | 평가 데이터나 미래 정답 정보가 학습 과정에 새어 들어가는 일 |
| 평활화(Smoothing) | 불안정한 값을 전체 평균 쪽으로 부드럽게 보정하는 일 |
| 분포 변환 | 값들이 퍼진 모양을 바꾸는 일 |
| [로그 변환](#5-로그-변환) | 큰 값을 더 강하게 압축하는 변환 |
| 구간화(Binning) | 연속적인 값을 여러 구간으로 나누는 일 |
| 요약 변수 | 여러 행을 묶어 만든 변수 |
| 파생 변수 | 기존 변수를 조합해 새로 만든 변수 |

## 핵심 개념 링크

- [정규화](#2-정규화와-표준화): 값의 범위를 보통 0과 1 사이로 맞추는 변환
- [표준화](#2-정규화와-표준화): 평균을 0, 표준편차를 1 기준으로 맞추는 변환
- [인코딩](#3-인코딩): 범주나 문자를 모델이 읽을 수 있는 숫자 코드로 바꾸는 일
- [로그 변환](#5-로그-변환): 큰 값을 더 강하게 압축하는 변환
- [구간화](#7-구간화): 연속적인 값을 여러 구간 상자에 나누는 변환

## 용어 이름 먼저 풀기

영어 용어는 이름의 느낌을 알면 덜 낯설다.

| 용어 | 이름의 뉘앙스 |
|---|---|
| Transformation | `form`을 바꾼다는 말이다. 값의 본질을 없애는 것이 아니라 표현 형태를 바꾼다. |
| Normalization | 제각각인 값을 하나의 `normal`, 즉 공통 기준에 맞춘다는 느낌이다. |
| Encoding | 정보를 컴퓨터가 읽는 `code`로 바꾼다는 말이다. |
| Leakage | 물이 새듯이, 아직 쓰면 안 되는 정보가 학습 과정에 새어 들어간다는 말이다. |
| Binning | 값을 여러 `bin`, 즉 상자에 나누어 담는다는 말이다. |
| Smoothing | 들쭉날쭉한 값을 부드럽게 만들어 과한 흔들림을 줄인다는 말이다. |
...
이 차시에서는 “로그 변환은 오른쪽 꼬리를 줄인다”처럼 결론만 외우지 않는다. 왜 그런 일이 생기는지를 본다.
...
- **이전에서 이어지는 점**: 2차시에서 문제 있는 값을 다뤘다면, 이제 값의 형태 자체를 분석하기 좋게 바꾼다.
- **다음으로 이어지는 점**: 4차시에서는 변환된 데이터를 해석하는 통계와 확률 언어를 배운다.
...
데이터 변환은 데이터를 분석하기 좋은 형태로 바꾸는 작업이다. 수학적으로는 기존 변수 공간보다 문제를 더 쉽게 풀 수 있는 공간으로 옮기는 일이라고 볼 수 있다.
...
하지만 모든 변환은 대가가 있다. 정규화는 원래 단위를 흐리게 만들고, 구간화는 세밀한 차이를 버리며, 인코딩은 범주에 숫자 의미를 부여할 수 있다. 따라서 변환은 자동 적용 목록이 아니라 판단의 문제다.
