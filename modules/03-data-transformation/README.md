...
## 원본 강의자료 보강: 인코딩과 분포 변환 선택

원본 PDF는 변환을 스케일 변환, 인코딩, 분포 변환, 구간화, 변수 생성으로 나눈다. 스케일 변환은 변수 단위를 맞추는 작업이고, 인코딩은 컴퓨터가 바로 계산하기 어려운 범주나 텍스트를 수치 표현으로 바꾸는 작업이다. 분포 변환은 모델 가정이나 이상치 영향 때문에 값의 모양을 바꾸는 작업이다.
