...
- 원래 단위, 상대적 위치, 표준편차 단위가 어떻게 다른지 구분할 수 있다.
...
여기서 `x`는 입력 변수 벡터, `w`는 각 변수에 곱해지는 가중치, `b`는 기준을 이동시키는 절편이다. `w^T x + b`로 데이터를 한 숫자 점수로 바꾼 뒤, 그 값이 0 이상이면 1, 0보다 작으면 0으로 나눈다. 변환의 목적은 이처럼 원래 공간에서 복잡한 문제를 더 판단하기 쉬운 공간으로 옮기는 것이다.
...
변환 전에는 항상 원래 단위를 적어 둔다. 원 단위는 해석에 필요하고, 변환 단위는 모델 학습이나 통계 가정을 맞추는 데 필요하다. 예를 들어 매출을 로그 변환해 모델에 넣더라도, 최종 설명은 “로그 매출이 0.2 증가했다”에서 멈추지 않고 실제 매출 변화의 의미로 다시 풀어야 한다.
...
Z-score 표준화는 “평균에서 얼마나 멀리 떨어졌는가”를 보여 준다. 값이 2라면 평균보다 표준편차 2개만큼 크다는 뜻이다. 이 방식은 단위가 달라도 상대적 위치를 비교하기 좋지만, 원래 단위의 직관은 약해진다.

정규화와 표준화는 이름이 비슷하지만 질문이 다르다. Min-Max 정규화는 “최솟값과 최댓값 사이에서 어디쯤인가?”를 묻고, Z-score 표준화는 “평균에서 표준편차 몇 개만큼 떨어졌는가?”를 묻는다. 5강과 6강의 표준화는 주로 후자의 감각과 연결된다. 값을 표준편차 단위로 바꾸면 서로 다른 단위의 변수나 검정통계량을 같은 기준 분포와 비교할 수 있다.
...
레이블 인코딩이 위험한 이유는 숫자가 순서를 만들기 때문이다. `서울=0`, `부산=1`, `대구=2`로 바꾸면 모델은 대구가 서울보다 두 배 크다는 식의 잘못된 신호를 받을 수 있다. 이 범주들 사이에는 크기 순서가 없으므로 원-핫 인코딩이 더 자연스러운 경우가 많다.

타깃 인코딩은 강력하지만 조심해야 한다. 예를 들어 상품군별 구매율을 인코딩한다고 하자. 특정 상품군이 훈련 데이터에서 우연히 구매율이 높게 나왔을 수 있다. 이 값을 그대로 쓰면 모델은 우연을 진짜 규칙으로 외울 수 있다.
...
분포 변환은 통계적으로 더 바람직한 형태로 값을 바꾸는 작업이다. 목적은 보통 네 가지다.
...
로그 변환 후 차이는 원래 값의 절대 차이보다 비율 차이에 가깝다. 예를 들어 로그 매출 차이가 같다는 것은 원래 매출이 같은 금액만큼 차이 난다는 뜻이 아니라, 비슷한 배율로 차이 난다는 뜻에 가깝다. 그래서 로그 변환은 매출, 소득, 방문 수처럼 “몇 배 증가” 해석이 자연스러운 변수에서 특히 자주 쓰인다.
...
Box-Cox 변환과 Yeo-Johnson 변환은 적절한 `lambda`를 찾는 방식이다. Box-Cox는 양수 데이터에 주로 쓰이고, Yeo-Johnson은 0이나 음수가 있어도 쓸 수 있다는 차이가 있다.

Box-Cox 변환은 다음처럼 정의한다.
...
`lambda`가 0에 가까워질 때는 로그 변환과 연결된다. Box-Cox는 여러 `lambda` 중 데이터가 정규분포에 가장 가까워지도록 하는 값을 찾는다. 그래서 정규성을 가정하는 회귀나 검정 방법의 안정성을 높이는 데 도움을 줄 수 있다.

Yeo-Johnson 변환은 0 이상과 음수 값을 나누어 처리한다.

```text
if y_i >= 0:
  y_i(lambda) = ((y_i + 1)^lambda - 1) / lambda,  if lambda != 0
              = ln(y_i + 1),                      if lambda = 0

if y_i < 0:
  y_i(lambda) = -(((-y_i + 1)^(2 - lambda) - 1) / (2 - lambda)),  if lambda != 2
              = -ln(-y_i + 1),                                      if lambda = 2
```

처음 학습에서는 “Box-Cox는 양수 데이터에서 적절한 거듭제곱을 찾고, Yeo-Johnson은 0이나 음수까지 포함할 수 있게 확장한 방법”으로 잡으면 된다.

### 7. 구간화와 이산화

구간화는 연속형 변수를 범주형 변수로 바꾸는 일이다. Binning 또는 Discretization, 즉 이산화라고도 부른다. 이름처럼 연속적인 값을 여러 상자에 나누어 담는다.
...
요약 변수는 여러 행을 묶어서 만든 변수다. 예를 들어 거래 단위 데이터를 고객 단위로 바꿀 때 `최근 구매일`, `구매 빈도`, `총 구매액`을 만들 수 있다.

파생 변수는 기존 변수를 조합해 새로 만든 변수다. 예를 들어 키와 몸무게로 BMI를 만들거나, 매출과 방문자 수로 방문자당 매출을 만들 수 있다.
...
데이터 변환은 스케일 변환, 인코딩, 분포 변환, 구간화, 변수 생성으로 나눈다. 스케일 변환은 변수 단위를 맞추는 작업이고, 인코딩은 컴퓨터가 바로 계산하기 어려운 범주나 텍스트를 수치 표현으로 바꾸는 작업이다. 분포 변환은 모델 가정이나 이상치 영향 때문에 값의 모양을 바꾸는 작업이다.

빈도 인코딩은 범주를 등장 횟수나 비율로 바꾼다. 타깃 인코딩은 범주별 목표값 평균을 쓰기 때문에 강력하지만 정답 정보가 새어 들어가기 쉽다. 그래서 교차검증 방식과 평활화가 중요하다. 평활화는 표본 수가 적은 범주의 평균을 전체 평균 쪽으로 당겨 우연한 극단값을 줄인다.

로그 변환, 지수 변환, 거듭제곱 변환, Box-Cox 변환은 모두 분포 모양을 바꾸지만 방향이 다르다. 로그는 큰 양수 값을 압축하고, 지수 변환은 큰 값을 더 벌릴 수 있다. Box-Cox는 양수 데이터에서 적절한 거듭제곱을 찾고, Yeo-Johnson은 0이나 음수까지 포함할 수 있다.
...
- <a id="note-03-구간화"></a>[구간화](#ref-03-구간화): 연속적인 값을 여러 구간 상자에 나누는 변환. Binning 또는 Discretization, 즉 이산화라고도 부른다.
