...
| 하이퍼파라미터 | 모델이 학습하기 전에 사람이 정하는 설정 |
| 그리드 서치 | 후보 조합을 격자처럼 모두 탐색하는 방법 |
| 랜덤 서치 | 후보 조합을 무작위로 뽑아 탐색하는 방법 |
| 속성 선택 | 모델에 사용할 변수를 고르는 과정 |
| 클래스 불균형 | 어떤 클래스의 표본 수가 다른 클래스보다 매우 적은 상태 |
| SMOTE | 소수 클래스의 합성 표본을 만드는 오버샘플링 방법 |
| ADASYN | 분류가 어려운 소수 클래스 주변에 합성 표본을 더 만드는 방법 |
...
| SMOTE | 소수 클래스 사이를 보간해 synthetic, 즉 합성 표본을 만든다. |
...
- **클래스 불균형 지표**: 소수 클래스가 중요한 문제에서는 정확도보다 정밀도, 재현율, PR-AUC가 더 적절할 수 있다.
...
![하이퍼파라미터 탐색](assets/hyperparameter-search-491.png)
...
정확도보다 재현율, 정밀도, F1, PR-AUC 등을 본다. 어떤 오류가 더 비싼지에 따라 임계값도 조정한다.

PR-AUC는 Precision-Recall 곡선 아래 면적이다. 양성 클래스가 드문 문제에서는 “찾아낸 양성이 얼마나 믿을 만한가”와 “실제 양성을 얼마나 놓치지 않았는가”를 함께 보므로 ROC-AUC보다 상황을 더 잘 보여 줄 수 있다.
...
사기 거래가 1%라면 정확도보다 재현율, 정밀도, PR-AUC를 보고 클래스 가중치나 SMOTE를 검토한다.
