...
### 5. 원본 강의자료 보강: 속성 선택과 불균형 대응 선택

원본 PDF는 속성 선택을 Filter, Wrapper, Embedded 방식으로 구분한다. Filter는 모델을 학습하기 전에 상관, 카이제곱 검정, ANOVA F값 같은 기준으로 빠르게 걸러낸다. Wrapper는 전진 선택법이나 후진 제거법처럼 실제 모델 성능을 보며 변수 조합을 탐색한다. Embedded는 Lasso나 트리 기반 중요도처럼 모델 학습 과정 안에서 선택이 함께 일어난다.

클래스 불균형 대응은 하나만 정답이 아니다. 클래스 가중치는 소수 클래스 오류에 더 큰 손실을 주고, 임계값 조정은 모델이 낸 확률을 어디서 양성으로 자를지 바꾼다. 오버샘플링은 소수 클래스를 늘리고, 언더샘플링은 다수 클래스를 줄인다. SMOTE는 소수 클래스 이웃 사이를 보간하고, ADASYN은 더 어려운 소수 클래스 주변을 더 많이 보강한다.
