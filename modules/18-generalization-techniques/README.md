...
- **<a id="ref-18-성능-지표"></a>[성능 지표](#note-18-성능-지표)**: 불균형에서 정확도만 보면 위험한 이유
- **샘플링**: 훈련 데이터 안에서만 <a id="ref-18-분포"></a>[분포](#note-18-분포)를 바꿔야 하는 이유
...
- **필수**: <a id="ref-18-하이퍼파라미터"></a>[하이퍼파라미터](#note-18-하이퍼파라미터)와 파라미터 구분, <a id="ref-18-속성-선택"></a>[속성 선택](#note-18-속성-선택)의 세 방식, 불균형 데이터에서 지표와 샘플링 선택
- **심화**: <a id="ref-18-smote"></a>[SMOTE](#note-18-smote)와 <a id="ref-18-adasyn"></a>[ADASYN](#note-18-adasyn)의 합성 <a id="ref-18-표본"></a>[표본](#note-18-표본) 차이
- **나중**: 중첩 <a id="ref-18-교차검증"></a>[교차검증](#note-18-교차검증)과 임계값 최적화
...
- **속성 선택**: 모든 <a id="ref-18-변수"></a>[변수](#note-18-변수)를 넣기보다 예측에 도움이 되는 변수만 남기면 해석과 <a id="ref-18-일반화"></a>[일반화](#note-18-일반화)가 좋아질 수 있다.
- **클래스 불균형 지표**: 소수 클래스가 중요한 문제에서는 정확도보다 정밀도, 재현율, F1을 더 중요하게 봐야 할 수 있다.
- **샘플링과 가중치**: 소수 클래스를 더 자주 보게 하거나 더 큰 손실을 주어 모델이 무시하지 않게 만든다.

### 1. 하이퍼파라미터 탐색

<a id="ref-18-그리드-서치"></a>[그리드 서치](#note-18-그리드-서치)는 후보 조합을 모두 본다. <a id="ref-18-랜덤-서치"></a>[랜덤 서치](#note-18-랜덤-서치)는 넓은 공간을 빠르게 탐색한다. 탐색은 검증 데이터나 교차검증으로 평가해야 한다.
...
- <a id="note-18-성능-지표"></a>[성능 지표](#ref-18-성능-지표): 모델 결과를 평가하는 숫자. ([처음 설명된 차시](../11-machine-learning/README.md#2-성능-지표))
- <a id="note-18-분포"></a>[분포](#ref-18-분포): 값들이 어떤 모양으로 흩어져 있는지 나타내는 구조. ([처음 설명된 차시](../05-probability-distributions/README.md#1-확률변수와-분포))
- <a id="note-18-하이퍼파라미터"></a>[하이퍼파라미터](#ref-18-하이퍼파라미터): 모델이 학습하기 전에 사람이 정하는 설정.
- <a id="note-18-속성-선택"></a>[속성 선택](#ref-18-속성-선택): 모델에 사용할 변수를 고르는 과정.
- <a id="note-18-smote"></a>[SMOTE](#ref-18-smote): 소수 클래스의 합성 표본을 만드는 오버샘플링 방법. 이름: 소수 클래스 사이를 보간해 synthetic, 즉 합성 표본을 만든다.
- <a id="note-18-adasyn"></a>[ADASYN](#ref-18-adasyn): 분류가 어려운 소수 클래스 주변에 합성 표본을 더 만드는 방법.
...
- <a id="note-18-일반화"></a>[일반화](#ref-18-일반화): 새 데이터에서도 잘 작동하는 성질.
- <a id="note-18-그리드-서치"></a>[그리드 서치](#ref-18-그리드-서치): 후보 조합을 격자처럼 모두 탐색하는 방법.
- <a id="note-18-랜덤-서치"></a>[랜덤 서치](#ref-18-랜덤-서치): 후보 조합을 무작위로 뽑아 탐색하는 방법.
