...
## 원본 강의자료 보강: 표본추출과 확률 법칙

원본 PDF는 표본 추출법을 확률 표본 추출과 비확률 표본 추출로 나눈다. 단순임의추출은 모든 대상이 같은 확률로 뽑히도록 하고, 계통추출은 일정 간격으로 뽑는다. 층화추출은 중요한 하위집단을 나눈 뒤 각 층에서 뽑아 대표성을 높이고, 집락추출은 학교나 지점 같은 묶음 일부를 뽑아 조사 비용을 줄인다.
