...
| 용어 | 먼저 이렇게 이해하기 |
|---|---|
| 통계 | 데이터를 정리하고 해석해 결론을 내는 방법 |
| 기술 통계 | 가지고 있는 데이터를 요약하는 방법 |
| 추론 통계 | 일부 데이터로 전체에 대해 말하는 방법 |
| [모집단](#2-모집단과-표본) | 알고 싶은 전체 대상 |
| [표본](#2-모집단과-표본) | 전체 대신 관찰한 일부 대상 |
| 표본 추출 | 모집단에서 표본을 고르는 과정 |
| 중심 경향 | 값들이 어느 위치에 모이는지 |
| 산포도 | 값들이 얼마나 퍼져 있는지 |
| [왜도](#6-분포의-모양) | 분포가 어느 한쪽으로 치우친 정도 |
| 첨도 | 분포의 꼬리와 뾰족함의 정도 |
| 시행 | 우연이 포함된 실험이나 행위 |
| 표본공간 | 가능한 모든 결과의 모음 |
| 사건 | 관심 있는 결과의 묶음 |
| [조건부 확률](#9-조건부-확률) | 어떤 일이 일어났다고 알고 난 뒤 다시 계산한 확률 |
| [베이즈 정리](#11-베이즈-정리) | 조건을 거꾸로 바꿔 계산하는 규칙 |

## 핵심 개념 링크

- [모집단](#2-모집단과-표본): 알고 싶은 전체 대상
- [표본](#2-모집단과-표본): 전체 대신 관찰한 일부 대상
- [평균](#4-중심-경향): 모든 값을 더해 개수로 나눈 대표값
- [왜도](#6-분포의-모양): 분포가 어느 한쪽으로 치우친 정도
- [조건부 확률](#9-조건부-확률): 어떤 일이 일어났다고 알고 난 뒤 다시 계산한 확률
- [베이즈 정리](#11-베이즈-정리): 조건을 거꾸로 바꿔 계산하는 규칙
...
| 용어 | 이름의 뉘앙스 |
|---|---|
| Descriptive Statistics | `describe`, 즉 데이터를 묘사하고 요약한다는 뜻이다. |
| Inferential Statistics | `infer`, 즉 일부를 보고 보이지 않는 전체를 추론한다는 뜻이다. |
| Population | 관심 대상 전체 집단이다. 사람만 뜻하지 않고 상품, 거래, 환자 전체도 될 수 있다. |
| Sample | 전체에서 떼어 낸 작은 부분이다. |
| Sampling | 표본을 뽑는 과정이다. |
| Skewness | `skew`는 비뚤어짐, 기울어짐의 느낌이다. 분포가 한쪽으로 치우친 정도를 말한다. |
| Conditional Probability | `condition`, 즉 조건이 붙은 확률이다. 이미 아는 조건 안에서 다시 계산한다. |
| Bayes's Theorem | 베이즈라는 사람이 정리한 정리다. 핵심은 `A일 때 B`와 `B일 때 A`를 연결하는 것이다. |
...
- **이전에서 이어지는 점**: 1~3차시에서 데이터 표를 다뤘다면, 이제 표본으로 전체를 말하는 언어를 배운다.
- **다음으로 이어지는 점**: 5차시에서는 확률이 값들의 모양, 즉 분포로 어떻게 표현되는지 본다.
...
| 지표 | 쉬운 의미 | 해석 |
|---|---|---|
| [왜도](#6-분포의-모양) | 좌우 비대칭 정도 | 양의 왜도는 오른쪽 꼬리, 음의 왜도는 왼쪽 꼬리 |
...
확률의 범위는 0에서 1 사이다. 일어날 수 없는 사건의 확률은 0이고, 표본공간 전체의 확률은 1이다.
