...
p-value가 작다는 것은 귀무가설이 맞는 세계에서는 지금 결과가 드물다는 뜻이다. <a id="ref-06-유의수준"></a>[유의수준](#note-06-유의수준) alpha는 “이 정도로 드물면 기본 주장을 버리자”는 기준이다. alpha가 0.05라면 p-value가 0.05보다 작을 때 기각한다.
...
표본평균, 표본분산, 표본비율은 그대로 비교하기 어렵다. 그래서 표준화된 <a id="ref-06-검정통계량"></a>[검정통계량](#note-06-검정통계량)으로 바꾼다. <a id="ref-06-기각역"></a>[기각역](#note-06-기각역)은 그 통계량이 어느 영역에 들어가면 귀무가설을 버릴지 미리 정한 구간이다.
