...
- **[분포](../05-probability-distributions/README.md#1-확률변수와-분포)**: 검정통계량이 얼마나 드문지 판단하는 기준
- **[표본평균](../05-probability-distributions/README.md#4-기대값-분산-중심극한정리)**: 표본 결과를 하나의 판단 숫자로 바꾸는 재료
- **[조건부 확률](../04-statistics-probability/README.md#9-조건부-확률)**: p-value를 “가정했을 때”의 확률로 읽는 감각

## 처음 보는 단어

| 용어 | 먼저 이렇게 이해하기 |
|---|---|
| 가설 검정 | 두 주장 중 어느 쪽을 증거로 판단할지 정하는 절차 |
| 귀무가설 | 일단 맞다고 놓고 반박하려는 기본 주장 |
| 대립가설 | 데이터로 보이고 싶은 변화나 차이의 주장 |
| [p-value](#2-p-value와-유의수준) | 귀무가설 아래에서 지금 결과가 얼마나 드문지 |
| [유의수준](#2-p-value와-유의수준) | 얼마나 드문 결과부터 귀무가설을 버릴지 정한 기준 |
| [기각역](#3-검정통계량과-기각역) | 귀무가설을 기각하는 통계량의 영역 |
| [검정통계량](#3-검정통계량과-기각역) | 표본 결과를 하나의 판단 숫자로 바꾼 값 |
| [양측검정](#4-단측검정과-양측검정) | 차이가 어느 방향이든 있는지 보는 검정 |
...
| 용어 | 이름의 뉘앙스 |
|---|---|
| Null Hypothesis | null은 “효과 없음, 차이 없음”의 느낌이다. 기본 상태를 뜻한다. |
| Alternative Hypothesis | alternative는 대안이다. 기본 주장 대신 받아들이고 싶은 주장이다. |
| [p-value](#2-p-value와-유의수준) | probability value의 줄임말이다. 귀무가설 아래에서 결과가 얼마나 드문지 보는 확률값이다. |
| Significance Level | significant는 의미 있다고 판단할 기준이다. 보통 alpha로 적는다. |
| Rejection Region | reject는 버린다는 뜻이다. 귀무가설을 버리는 영역이다. |
...
- **이전에서 이어지는 점**: 5차시에서 배운 분포를 기준선으로 삼아 표본 결과가 드문지 판단한다.
- **다음으로 이어지는 점**: 7차시에서는 버릴지 말지의 판단을 넘어 가능한 값의 범위를 말한다.
...
검정은 표본으로 모집단의 주장에 답하는 방법이다. 예를 들어 우유 200ml가 실제로 부족한지 알고 싶다면, 먼저 “평균은 200ml다”를 기본 주장으로 놓고 표본이 이 주장을 얼마나 흔드는지 본다.
