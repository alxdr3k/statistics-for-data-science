...
| p-value | 귀무가설 아래에서 지금 결과가 얼마나 드문지 |
| 유의수준 | 얼마나 드문 결과부터 귀무가설을 버릴지 정한 기준 |
| 기각역 | 귀무가설을 기각하는 통계량의 영역 |
| 검정통계량 | 표본 결과를 하나의 판단 숫자로 바꾼 값 |
| 양측검정 | 차이가 어느 방향이든 있는지 보는 검정 |
...
| p-value | probability value의 줄임말이다. 귀무가설 아래에서 결과가 얼마나 드문지 보는 확률값이다. |
