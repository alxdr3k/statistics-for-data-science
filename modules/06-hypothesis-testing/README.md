...
- **<a id="ref-06-분포"></a>[분포](#note-06-분포)**: <a id="ref-06-검정통계량"></a>[검정통계량](#note-06-검정통계량)이 얼마나 드문지 판단하는 기준
- **표본평균**: <a id="ref-06-표본"></a>[표본](#note-06-표본) 결과를 하나의 판단 숫자로 바꾸는 재료
- **<a id="ref-06-조건부-확률"></a>[조건부 확률](#note-06-조건부-확률)**: <a id="ref-06-p-value"></a>[p-value](#note-06-p-value)를 “가정했을 때”의 확률로 읽는 감각
...
- **필수**: <a id="ref-06-귀무가설"></a>[귀무가설](#note-06-귀무가설)과 <a id="ref-06-대립가설"></a>[대립가설](#note-06-대립가설) 구분, p-value와 <a id="ref-06-유의수준"></a>[유의수준](#note-06-유의수준)의 의미, 단측/양측 검정을 질문에서 결정
- **심화**: 1종/2종 오류의 균형
- **나중**: 검정력과 표본수 설계

## 이 차시에서 꼭 붙잡을 설명 방식

p-value를 “귀무가설이 맞을 확률”로 해석하면 틀린다. p-value는 귀무가설이 맞다고 이미 가정한 뒤, 그런 세계에서 지금 같은 표본 결과가 얼마나 드문지를 계산한다. 즉 방향이 반대다. 이 차이를 놓치면 0.03이라는 숫자를 “귀무가설이 3% 확률로 참”이라고 오해한다.

## 핵심 이론

### 먼저 잡는 직관

- **<a id="ref-06-가설-검정"></a>[가설 검정](#note-06-가설-검정)의 출발점**: 검정은 “기본 주장이 맞다”고 일단 놓고, 표본이 그 주장을 얼마나 흔드는지 보는 절차다.
- **p-value와 유의수준**: p-value는 귀무가설이 맞는 세계에서 지금 결과가 얼마나 드문지이고, 유의수준은 그 드묾을 판단하는 기준선이다.
- **검정통계량과 <a id="ref-06-기각역"></a>[기각역](#note-06-기각역)**: 표본 결과를 바로 비교하기 어렵기 때문에 하나의 판단 숫자로 바꾸고, 그 숫자가 기각역에 들어가는지 본다.
- **단측검정과 <a id="ref-06-양측검정"></a>[양측검정](#note-06-양측검정)**: 관심이 한쪽 방향인지, 어느 방향이든 차이인지에 따라 꼬리를 보는 방식이 달라진다.
...
표본평균, 표본분산, 표본비율은 그대로 비교하기 어렵다. 그래서 표준화된 검정통계량으로 바꾼다. 기각역은 그 통계량이 어느 영역에 들어가면 귀무가설을 버릴지 미리 정한 구간이다.
...
2. 귀무가설과 대립가설을 방향까지 포함해 정한다.
3. 검정통계량이 어떤 분포를 기준으로 판단되는지 확인한다.
...
- <a id="note-06-조건부-확률"></a>[조건부 확률](#ref-06-조건부-확률): 어떤 일이 일어났다고 알고 난 뒤 다시 계산한 확률. ([처음 설명된 차시](../04-statistics-probability/README.md#9-조건부-확률))
...
- <a id="note-06-유의수준"></a>[유의수준](#ref-06-유의수준): 얼마나 드문 결과부터 귀무가설을 버릴지 정한 기준.
- <a id="note-06-가설-검정"></a>[가설 검정](#ref-06-가설-검정): 두 주장 중 어느 쪽을 증거로 판단할지 정하는 절차.
- <a id="note-06-기각역"></a>[기각역](#ref-06-기각역): 귀무가설을 기각하는 통계량의 영역.
- <a id="note-06-양측검정"></a>[양측검정](#ref-06-양측검정): 차이가 어느 방향이든 있는지 보는 검정.
