...
대립가설을 채택하려면 단순히 표본평균이 조금 작다는 정도가 아니라, 귀무가설의 기본 입장을 흔들 만큼 강한 증거가 필요하다.
...
```text
p-value = P(reject H0 or more extreme evidence | H0 is true)
```

더 엄밀하게는 귀무가설을 만족하는 모수 공간에서 기각확률의 최댓값을 기준으로 본다.

```text
p-value = max_{theta in Theta0} P_theta(reject H0 | H0 is true)
```
