...
t-SNE는 모든 거리를 똑같이 보존하려 하지 않는다. 가까운 이웃이 낮은 차원에서도 가깝게 보이도록 확률 분포를 맞춘다. 그래서 손글씨 숫자처럼 복잡한 고차원 데이터를 2차원으로 볼 때 유용하지만, 축의 방향이나 멀리 떨어진 군집 사이의 절대 거리는 조심해서 해석해야 한다.
