...
- **분산**: <a id="ref-17-pca"></a>[PCA](#note-17-pca)가 많이 퍼진 방향을 정보로 보는 관점
- **공분산**: 변수들이 함께 움직이는 정도
- **<a id="ref-17-시각화"></a>[시각화](#note-17-시각화)**: 고차원 구조를 낮은 차원에 보여 주는 목적
...
- **필수**: PCA는 <a id="ref-17-변수"></a>[변수](#note-17-변수) 선택이 아니라 새 축을 만드는 것, 설명분산비로 남길 성분 수 판단, <a id="ref-17-t-sne"></a>[t-SNE](#note-17-t-sne) 축 자체를 과해석하지 않기
- **심화**: <a id="ref-17-svd"></a>[SVD](#note-17-svd)와 PCA의 연결
- **나중**: <a id="ref-17-lda"></a>[LDA](#note-17-lda)와 <a id="ref-17-pls"></a>[PLS](#note-17-pls)의 지도 정보 활용

## 이 차시에서 꼭 붙잡을 설명 방식

PCA는 “변수를 그냥 버리는 것”이 아니다. 데이터가 가장 많이 퍼진 방향을 새 축으로 만들고, 그 축을 앞에서부터 사용한다. 그래서 원래 변수 여러 개가 섞인 새 변수, 즉 주성분이 생긴다.
...
- **<a id="ref-17-mds"></a>[MDS](#note-17-mds)와 t-SNE**: MDS와 t-SNE는 점들 사이의 거리나 이웃 관계가 낮은 차원에서도 비슷하게 보이도록 배치한다.
- **LDA와 PLS**: LDA는 클래스 분리를 잘 보이게 하고, PLS는 입력과 출력의 공분산을 잘 설명하는 축을 찾는다.

### 1. PCA

공분산 구조를 이용해 분산을 많이 설명하는 축을 찾는다. <a id="ref-17-고유값"></a>[고유값](#note-17-고유값)은 각 주성분이 설명하는 분산의 크기이고, <a id="ref-17-고유벡터"></a>[고유벡터](#note-17-고유벡터)는 방향이다.
...
- <a id="note-17-시각화"></a>[시각화](#ref-17-시각화): 숫자를 그래프나 그림으로 바꿔 보는 일. ([처음 설명된 차시](../01-data-understanding/README.md#8-시각화는-변수-타입과-질문의-함수다))
- <a id="note-17-변수"></a>[변수](#ref-17-변수): 관측 대상의 특징을 적어 둔 열. ([처음 설명된 차시](../01-data-understanding/README.md#4-단위-변수-관측치))
- <a id="note-17-t-sne"></a>[t-SNE](#ref-17-t-sne): 가까운 이웃 구조를 시각화하는 비선형 차원 축소. 이름: 이웃 관계를 확률로 바꾼 뒤 낮은 차원에서 비슷하게 맞추는 시각화 방법이다.
- <a id="note-17-svd"></a>[SVD](#ref-17-svd): 행렬을 중요한 구조로 분해하는 방법.
- <a id="note-17-lda"></a>[LDA](#ref-17-lda): 클래스 구분이 잘 되는 축을 찾는 지도 차원 축소.
- <a id="note-17-pls"></a>[PLS](#ref-17-pls): 입력과 출력의 공분산을 잘 설명하는 성분을 찾는 방법.
- <a id="note-17-mds"></a>[MDS](#ref-17-mds): 거리 관계를 낮은 차원에 배치하는 방법.
- <a id="note-17-고유값"></a>[고유값](#ref-17-고유값): 각 주성분이 설명하는 분산의 크기.
- <a id="note-17-고유벡터"></a>[고유벡터](#ref-17-고유벡터): 새 축의 방향을 나타내는 벡터.
