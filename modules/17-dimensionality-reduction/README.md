...
| PCA | 분산을 가장 많이 보존하는 새 축을 찾는 방법 |
| 고유값 | 각 주성분이 설명하는 분산의 크기 |
| 고유벡터 | 새 축의 방향을 나타내는 벡터 |
| SVD | 행렬을 중요한 구조로 분해하는 방법 |
| MDS | 거리 관계를 낮은 차원에 배치하는 방법 |
| t-SNE | 가까운 이웃 구조를 시각화하는 비선형 차원 축소 |
| LDA | 클래스 구분이 잘 되는 축을 찾는 지도 차원 축소 |
...
| t-SNE | 이웃 관계를 확률로 바꾼 뒤 낮은 차원에서 비슷하게 맞추는 시각화 방법이다. |
