...
t-SNE는 모든 거리를 똑같이 보존하려 하지 않는다. 가까운 이웃이 낮은 차원에서도 가깝게 보이도록 확률 분포를 맞춘다. 그래서 MNIST 손글씨 숫자처럼 복잡한 고차원 데이터를 2차원으로 볼 때 유용하지만, 축의 방향이나 멀리 떨어진 군집 사이의 절대 거리는 조심해서 해석해야 한다.

PLS는 PCA와 달리 y를 무시하지 않는다. X의 분산만 큰 방향이 아니라, X와 y가 함께 잘 움직이는 방향을 찾는다. PLS2는 여러 개의 반응변수를 함께 다루고, PLS1은 단일 반응변수 문제를 다룬다. 예측 목적이 강하고 입력 변수들이 서로 많이 겹칠 때 PCA보다 PLS가 더 직접적일 수 있다.
