...
- **공분산**: 변수들이 함께 움직이는 정도
- **시각화**: 고차원 구조를 낮은 차원에 보여 주는 목적

## 처음 보는 단어

| 용어 | 먼저 이렇게 이해하기 |
|---|---|
| 차원 축소 | 많은 변수를 더 적은 축으로 표현하는 방법 |
| [PCA](#1-pca) | 분산을 가장 많이 보존하는 새 축을 찾는 방법 |
| 고유값 | 각 주성분이 설명하는 분산의 크기 |
| 고유벡터 | 새 축의 방향을 나타내는 벡터 |
| [SVD](#2-svd) | 행렬을 중요한 구조로 분해하는 방법 |
| MDS | 거리 관계를 낮은 차원에 배치하는 방법 |
| [t-SNE](#3-mds와-t-sne) | 가까운 이웃 구조를 시각화하는 비선형 차원 축소 |
| [LDA](#4-lda와-pls) | 클래스 구분이 잘 되는 축을 찾는 지도 차원 축소 |
...
- [t-SNE](#3-mds와-t-sne): 가까운 이웃 구조를 시각화하는 비선형 차원 축소
- [LDA](#4-lda와-pls): 클래스 구분이 잘 되는 축을 찾는 지도 차원 축소
- [PLS](#4-lda와-pls): 입력과 출력의 공분산을 잘 설명하는 성분을 찾는 방법

## 용어 이름 먼저 풀기

| 용어 | 이름의 뉘앙스 |
|---|---|
| Dimensionality Reduction | dimension은 축이나 변수의 수다. 많은 축을 줄인다는 뜻이다. |
| Principal Component | 가장 중요한 변동 방향이라는 뜻이다. |
| Eigenvector | 행렬 변환 후에도 방향이 유지되는 특별한 벡터다. |
| Embedding | 높은 차원의 대상을 낮은 차원에 심어 표현한다는 느낌이다. |
| [t-SNE](#3-mds와-t-sne) | 이웃 관계를 확률로 바꾼 뒤 낮은 차원에서 비슷하게 맞추는 시각화 방법이다. |
...
- **이전에서 이어지는 점**: 16차시에서 비슷한 점을 묶었다면, 17차시는 변수 수를 줄여 구조를 보기 쉽게 만든다.
- **다음으로 이어지는 점**: 18차시에서는 모델이 새 데이터에서도 잘 작동하도록 검증과 튜닝을 다룬다.
...
1. 차원 축소 목적이 시각화인지, 압축인지, 예측 전처리인지 구분한다.
2. PCA 전에는 변수 스케일이 주성분을 지배하지 않도록 표준화를 확인한다.
