...
### 5. 원본 강의자료 보강: EM 수행과 MCMC 진단

원본 PDF의 EM 예시는 가우시안 혼합분포에서 시작한다. 데이터가 여러 정규분포의 섞임에서 왔다고 보고, 각 데이터가 어느 성분에서 왔는지 모르는 상태를 숨은 정보로 둔다. E-step은 각 데이터가 각 성분에 속할 책임도, 즉 posterior weight를 계산하고, M-step은 그 책임도를 가중치처럼 사용해 평균, 분산, 혼합비를 다시 추정한다.
