...
여기서 숨은 소속처럼 직접 보이지 않는 값을 **<a id="ref-08-잠재변수"></a>[잠재변수](#note-08-잠재변수)**라고 부른다. 가우시안 혼합 모델에서는 데이터가 여러 정규분포 중 어느 <a id="ref-08-분포"></a>[분포](#note-08-분포)에서 왔는지가 잠재변수다. EM은 “소속을 알면 평균과 분산을 계산하기 쉽고, 평균과 분산을 알면 소속 가능성을 계산하기 쉽다”는 관계를 번갈아 이용한다.
...
<a id="ref-08-마르코프-체인"></a>[마르코프 체인](#note-08-마르코프-체인)은 현재 상태에서 다음 상태로 이동한다. <a id="ref-08-mcmc"></a>[MCMC](#note-08-mcmc)는 이 이동 규칙을 설계해 원하는 분포에서 나온 것 같은 표본을 얻는다.
