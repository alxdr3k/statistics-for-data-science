...
| 몬테카를로 | 무작위 샘플을 많이 뽑아 값을 추정하는 방법 |
| 마르코프 체인 | 현재 상태만 다음 상태에 영향을 주는 확률 과정 |
| 정상분포 | 충분히 이동한 뒤 상태 확률이 안정된 분포 |
| MCMC | 마르코프 체인으로 원하는 분포의 샘플을 얻는 방법 |
| burn-in | 시작점 영향이 큰 초반 샘플을 버리는 구간 |
| thinning | 너무 촘촘한 연속 샘플을 일정 간격으로 덜어 내는 방법 |
...
| Metropolis-Hastings | 제안한 이동을 받아들일지 확률적으로 결정해 목표 분포를 따라가게 하는 알고리즘이다. |
...
![마르코프 체인의 상태 전이](assets/markov-chain-222.png)
...
연속된 샘플은 상관될 수 있다. burn-in은 초반 표본을 버리는 일이고, thinning은 표본을 일정 간격으로 덜어 내는 일이다. 다만 thinning이 항상 필요한 것은 아니며, 핵심은 자기상관과 수렴을 점검하는 것이다.
