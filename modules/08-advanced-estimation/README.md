...
- **[분포](../05-probability-distributions/README.md#1-확률변수와-분포)**: 사후분포와 표본 추출의 대상
...
| 용어 | 먼저 이렇게 이해하기 |
|---|---|
| EM | 숨은 값을 추정하고 모수를 갱신하는 반복 알고리즘 |
| 잠재변수 | 직접 관측되지 않지만 모델 안에 숨어 있는 변수 |
| 가우시안 혼합 모델 | 여러 정규분포가 섞여 데이터를 만든다고 보는 모델 |
| [몬테카를로](#2-몬테카를로-방법) | 무작위 샘플을 많이 뽑아 값을 추정하는 방법 |
| 마르코프 체인 | 현재 상태만 다음 상태에 영향을 주는 확률 과정 |
| 정상분포 | 충분히 이동한 뒤 상태 확률이 안정된 분포 |
| [MCMC](#3-마르코프-체인과-mcmc) | 마르코프 체인으로 원하는 분포의 샘플을 얻는 방법 |
| burn-in | 시작점 영향이 큰 초반 샘플을 버리는 구간 |
| 자기상관 | 연속된 샘플끼리 서로 닮아 독립적이지 않은 정도 |
| 사전분포와 사후분포 | 데이터를 보기 전 믿음과 데이터를 본 뒤 갱신된 믿음 |

## 핵심 개념 링크

- [EM 알고리즘](#1-em-알고리즘): 숨은 값 추정과 모수 갱신을 번갈아 반복하는 알고리즘
- [몬테카를로](#2-몬테카를로-방법): 무작위 샘플을 많이 뽑아 값을 추정하는 방법
- [MCMC](#3-마르코프-체인과-mcmc): 마르코프 체인으로 원하는 분포의 샘플을 얻는 방법
- [Metropolis-Hastings](#4-metropolis-hastings): 후보를 제안하고 확률적으로 수락하며 원하는 분포를 탐색하는 MCMC 방법

## 용어 이름 먼저 풀기

| 용어 | 이름의 뉘앙스 |
|---|---|
| Expectation Maximization | 기대값을 계산하는 E step과 가능도를 최대화하는 M step이 번갈아 간다는 이름이다. |
| Gaussian Mixture Model | 여러 정규분포가 섞여 하나의 데이터 분포를 만든다는 뜻이다. |
| Monte Carlo | 무작위 시행을 많이 해서 값을 추정하는 방식이다. |
| Markov Chain | 현재 상태만 다음 상태에 영향을 주는 사슬 구조다. |
| Burn-in | 불이 안정되기 전 예열 구간처럼, 체인이 안정되기 전 초반 표본을 뜻한다. |
| [Metropolis-Hastings](#4-metropolis-hastings) | 제안한 이동을 받아들일지 확률적으로 결정해 목표 분포를 따라가게 하는 알고리즘이다. |
...
- **이전에서 이어지는 점**: 7차시의 추정이 직접 계산 가능한 상황이었다면, 8차시는 계산이 어려울 때 반복과 샘플링을 쓴다.
- **다음으로 이어지는 점**: 9차시에서는 추정된 모수를 사용해 변수 관계를 직선 모델로 설명한다.
...
1. 숨은 변수 때문에 직접 추정이 어려운 문제인지 먼저 확인한다.
2. 반복 알고리즘이 초기값에 민감할 수 있음을 염두에 둔다.
3. 몬테카를로 표본 수가 충분한지, 난수 변동이 결과를 바꾸지 않는지 본다.
4. MCMC에서는 burn-in, 자기상관, 수렴 진단을 확인한다. burn-in은 초반 표본을 버리는 일이고, 자기상관은 연속 표본이 서로 얼마나 닮았는지를 뜻한다.
