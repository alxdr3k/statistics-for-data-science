...
- 군집 결과가 실제 집단을 자동으로 증명하지 않는다는 점을 이해한다.
...
군집은 알고리즘이 만든 구조이지, 곧바로 현실의 참 집단을 뜻하지 않는다. 같은 데이터도 거리 척도, 스케일링, k값, 초기값에 따라 다른 군집이 나올 수 있다. 따라서 군집화 결과는 실루엣 점수 같은 숫자와 함께 업무적으로 해석 가능한지, 안정적으로 반복되는지 확인해야 한다.
...
k-means의 중심은 실제 데이터 포인트가 아닐 수 있다. 중심은 군집 안 점들의 평균이므로 “대표 고객”처럼 해석하면 위험할 수 있다. 반면 k-medoids의 medoid는 실제 관측치 중 대표점이라 설명이 더 쉬울 때가 있다.
...
DBSCAN에서 `eps`가 너무 작으면 많은 점이 잡음으로 빠지고, 너무 크면 서로 다른 군집이 하나로 붙을 수 있다. `min_samples`가 커질수록 더 조밀한 영역만 군집으로 인정한다. 그래서 DBSCAN은 파라미터가 자동으로 정답을 주는 모델이 아니라, 밀도 기준을 정해 주는 모델이다.
