...
- **<a id="ref-16-비지도학습"></a>[비지도학습](#note-16-비지도학습)**: 정답 없이 구조를 찾는 관점
- **거리**: 비슷함을 숫자로 재는 기준
- **스케일링**: 거리 계산이 <a id="ref-16-변수"></a>[변수](#note-16-변수) 단위에 영향을 받는 문제
...
- **필수**: <a id="ref-16-거리-척도"></a>[거리 척도](#note-16-거리-척도)가 군집 결과를 바꾼다는 점, <a id="ref-16-k-means"></a>[k-means](#note-16-k-means)의 k 선택과 초기값 민감성, <a id="ref-16-dbscan"></a>[DBSCAN](#note-16-dbscan)이 밀도와 잡음을 본다는 점
- **심화**: <a id="ref-16-gmm"></a>[GMM](#note-16-gmm)의 부드러운 소속 확률
- **나중**: 계층적 <a id="ref-16-군집화"></a>[군집화](#note-16-군집화)의 linkage 선택

## 이 차시에서 꼭 붙잡을 설명 방식

군집화에서 “가깝다”는 말은 자연스럽지만 사실 정의가 필요하다. 키와 몸무게를 그대로 쓰면 몸무게 단위가 더 크게 작용할 수 있고, 텍스트 벡터에서는 방향이 더 중요할 수 있다. 그래서 거리 <a id="ref-16-척도"></a>[척도](#note-16-척도)와 스케일링이 군집 결과를 좌우한다.
...
- **k-means와 <a id="ref-16-k-medoids"></a>[k-medoids](#note-16-k-medoids)**: k-means는 <a id="ref-16-평균"></a>[평균](#note-16-평균) 중심을 움직이고, k-medoids는 실제 데이터 대표점을 중심으로 삼는다.
...
- <a id="note-16-비지도학습"></a>[비지도학습](#ref-16-비지도학습): 정답 없이 데이터 구조를 찾는 학습. ([처음 설명된 차시](../11-machine-learning/README.md#1-학습-유형))
