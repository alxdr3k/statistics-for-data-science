...
- **분류**: 두 집단을 나누는 문제
- **거리**: <a id="ref-14-마진"></a>[마진](#note-14-마진)을 경계와 점 사이 거리로 읽기
- **과대적합**: C와 <a id="ref-14-커널"></a>[커널](#note-14-커널)이 경계 복잡도를 바꾼다는 감각
...
- **필수**: <a id="ref-14-최대-마진"></a>[최대 마진](#note-14-최대-마진)의 의미, <a id="ref-14-서포트-벡터"></a>[서포트 벡터](#note-14-서포트-벡터)가 경계를 결정한다는 점, C와 커널이 복잡도를 조절한다는 점
- **심화**: 라그랑지안과 <a id="ref-14-쌍대-문제"></a>[쌍대 문제](#note-14-쌍대-문제)
...
- **<a id="ref-14-슬랙"></a>[슬랙](#note-14-슬랙)과 C**: 현실 데이터는 완벽히 나뉘지 않으므로 일부 위반을 허용하고 그 위반을 얼마나 벌할지 정한다.
...
선형 경계로 나누기 어려운 데이터도 커널을 쓰면 더 복잡한 경계를 만들 수 있다. <a id="ref-14-rbf-커널"></a>[RBF 커널](#note-14-rbf-커널)의 <a id="ref-14-gamma"></a>[gamma](#note-14-gamma)가 크면 개별 점의 영향이 좁아져 결정면이 복잡해질 수 있다.
...
<a id="ref-14-커널-트릭"></a>[커널 트릭](#note-14-커널-트릭)은 고차원 내적 효과를 함수 계산으로 대신한다.
...
3. <a id="ref-14-슬랙-변수"></a>[슬랙 변수](#note-14-슬랙-변수)는 완벽히 분리되지 않는 데이터에서 마진 위반을 수치로 표현한다.
...
- <a id="note-14-마진"></a>[마진](#ref-14-마진): 결정경계와 가장 가까운 점 사이의 여백.
- <a id="note-14-커널"></a>[커널](#ref-14-커널): 두 데이터의 유사도를 계산하는 함수.
- <a id="note-14-최대-마진"></a>[최대 마진](#ref-14-최대-마진): 결정경계와 가장 가까운 점 사이 여백을 최대화하는 기준.
- <a id="note-14-서포트-벡터"></a>[서포트 벡터](#ref-14-서포트-벡터): 마진을 결정하는 경계 가까운 데이터.
- <a id="note-14-쌍대-문제"></a>[쌍대 문제](#ref-14-쌍대-문제): 원래 문제를 다른 변수 관점으로 바꾼 최적화 문제.
- <a id="note-14-슬랙"></a>[슬랙](#ref-14-슬랙): SVM에서 일부 분류 위반을 허용하는 여유 변수.
...
- <a id="note-14-커널-트릭"></a>[커널 트릭](#ref-14-커널-트릭): 고차원 변환 효과를 직접 좌표로 만들지 않고 계산하는 방법.
- <a id="note-14-슬랙-변수"></a>[슬랙 변수](#ref-14-슬랙-변수): 마진 위반을 허용하는 여유 변수.
