...
| [서포트 벡터](#2-서포트-벡터) | 마진을 결정하는 경계 가까운 데이터 |
| 슬랙 변수 | 마진 위반을 허용하는 여유 변수 |
| C | 마진 위반을 얼마나 강하게 벌할지 정하는 규제계수 |
| 쌍대 문제 | 원래 문제를 다른 변수 관점으로 바꾼 최적화 문제 |
| [커널](#4-커널) | 두 데이터의 유사도를 계산하는 함수 |
| [커널 트릭](#4-커널) | 고차원 변환 효과를 직접 좌표로 만들지 않고 계산하는 방법 |
| RBF 커널 | 가까운 점의 영향은 크게, 먼 점의 영향은 작게 보는 커널 |
| gamma | RBF 커널에서 한 점의 영향 범위를 조절하는 값 |

## 핵심 개념 링크

- [최대 마진](#1-최대-마진): 결정경계와 가장 가까운 점 사이 여백을 최대화하는 기준
- [서포트 벡터](#2-서포트-벡터): 마진을 결정하는 경계 가까운 데이터
- [슬랙](#3-슬랙과-c): SVM에서 일부 분류 위반을 허용하는 여유 변수
- [커널](#4-커널): 두 데이터의 유사도를 계산하는 함수
- [커널 트릭](#4-커널): 고차원 변환 효과를 직접 좌표로 만들지 않고 계산하는 방법
...
- **이전에서 이어지는 점**: 13차시에서 트리와 KNN을 봤다면, 14차시는 경계선을 가장 안정적으로 긋는 방법을 본다.
- **다음으로 이어지는 점**: 15차시에서는 하나의 모델보다 여러 모델을 합쳐 안정성을 높인다.
