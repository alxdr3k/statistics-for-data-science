...
SVM 수식은 부호와 거리를 나누어 읽는다. `w^T x + b`의 부호는 어느 클래스 쪽에 있는지를 말하고, 절댓값을 `||w||`로 나눈 값은 경계에서 얼마나 떨어졌는지를 말한다. 그래서 SVM은 단순히 맞혔는지만 보지 않고, 얼마나 여유 있게 맞혔는지도 본다.
...
입력 변수의 스케일도 중요하다. SVM은 거리와 내적을 사용하므로, 한 변수가 매우 큰 단위를 가지면 마진과 커널값을 지배할 수 있다. 그래서 SVM을 적용하기 전에는 표준화를 기본 점검으로 생각한다.
...
따라서 `C`는 “정답을 얼마나 엄격히 맞출 것인가”와 “마진을 얼마나 넓게 둘 것인가”의 균형이다. 큰 `C`는 훈련 오류를 줄일 수 있지만 복잡한 경계를 만들 수 있고, 작은 `C`는 일부 오류를 허용해 더 부드러운 경계를 만들 수 있다.
