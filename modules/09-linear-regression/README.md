...
- **<a id="ref-09-평균"></a>[평균](#note-09-평균)**: 회귀가 조건부 평균을 설명한다는 감각
- **분산**: <a id="ref-09-잔차"></a>[잔차](#note-09-잔차)의 퍼짐을 평가하는 기준
...
- **필수**: 회귀식의 입력/출력 관계, 잔차와 회귀 가정, <a id="ref-09-r2"></a>[R2](#note-09-r2)와 오차 지표의 차이
- **심화**: <a id="ref-09-aicbic"></a>[AIC/BIC](#note-09-aicbic)와 <a id="ref-09-다중공선성"></a>[다중공선성](#note-09-다중공선성)
- **나중**: 규제 회귀와 차원축소로의 확장

## 이 차시에서 꼭 붙잡을 설명 방식

잔차를 그냥 더하면 양수와 음수가 서로 상쇄된다. 그래서 오차를 제곱해 모두 양수로 만들고, 큰 오차를 더 크게 벌한다. <a id="ref-09-최소제곱법"></a>[최소제곱법](#note-09-최소제곱법)은 이 잔차 제곱합이 가장 작아지는 직선을 찾는 방법이다.

## 핵심 이론

### 먼저 잡는 직관

- **단순회귀와 다중회귀**: 회귀는 입력 <a id="ref-09-변수"></a>[변수](#note-09-변수)가 변할 때 평균적인 결과가 얼마나 움직이는지 직선식으로 표현한다.
...
선형성, 등분산성, 정규성, 독립성을 점검한다. 다중회귀에서는 독립변수끼리 너무 강하게 관련되는 다중공선성도 본다.

### 3. 모델 평가

<a id="ref-09-결정계수"></a>[결정계수](#note-09-결정계수) R2는 종속변수 변동 중 모델이 설명한 비율이다. 조정 R2는 변수가 늘어날 때의 과대평가를 줄이고, AIC/BIC는 적합도와 복잡도를 함께 본다.
...
1. <a id="ref-09-선형회귀"></a>[선형회귀](#note-09-선형회귀)는 변수 사이의 평균적 관계를 직선으로 단순화해 설명과 예측을 한다.
...
- <a id="note-09-평균"></a>[평균](#ref-09-평균): 모든 값을 더해 개수로 나눈 대표값. ([처음 설명된 차시](../04-statistics-probability/README.md#4-중심-경향))
...
- <a id="note-09-선형회귀"></a>[선형회귀](#ref-09-선형회귀): 입력과 출력의 평균적 관계를 직선식으로 설명하는 모델.
