...
- 회귀계수를 “다른 조건이 같을 때 한 단위 변화”로 해석한다.
...
회귀계수는 항상 단위와 조건을 함께 읽는다. 단순회귀에서 `beta1=3`이면 `X`가 1 증가할 때 `Y`의 평균이 3만큼 증가한다는 뜻이다. 다중회귀에서는 “다른 독립변수들이 같을 때”라는 조건이 붙는다. 이 조건을 빼면 계수를 단순 상관처럼 오해하기 쉽다.
...
여기서 `Y`는 실제값, `yhat`은 모델이 예측한 값, `error` 또는 잔차는 실제값과 예측값의 차이다.

```text
잔차 = 실제값 - 예측값 = y_i - yhat_i
```

잔차가 양수이면 모델이 실제보다 작게 예측한 것이고, 잔차가 음수이면 실제보다 크게 예측한 것이다. 잔차의 평균이 0에 가까워도 잔차에 곡선 패턴이나 부채꼴 패턴이 남아 있으면 모델 가정이 흔들릴 수 있다.
...
가정은 “모델을 쓰기 위한 자격시험”이라기보다 해석의 안전장치다. 예측만 목적이면 약간의 가정 위반을 검증 성능으로 관리할 수 있지만, 계수 p-value나 신뢰구간을 해석하려면 잔차 가정을 더 신중하게 봐야 한다.
