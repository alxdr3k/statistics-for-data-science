...
| [교호작용](#3-분산분석) | 한 요인의 효과가 다른 요인의 수준에 따라 달라지는 현상 |
| [카이제곱 검정](#4-카이제곱-검정) | 관측빈도와 기대빈도의 차이를 보는 검정 |
| 기대빈도 | 독립이나 가정이 맞다면 기대되는 빈도 |

## 핵심 개념 링크

- [상관 분석](#2-상관-분석): 두 변수가 함께 움직이는 방향과 강도를 재는 분석
- [분산분석](#3-분산분석): 여러 집단 평균 차이를 변동 분해로 검정하는 방법
- [카이제곱 검정](#4-카이제곱-검정): 관측빈도와 기대빈도의 차이를 보는 검정
- [교호작용](#3-분산분석): 한 요인의 효과가 다른 요인의 수준에 따라 달라지는 현상
...
- **이전에서 이어지는 점**: 9차시가 수치 변수 사이 직선 관계를 봤다면, 10차시는 변수 형태에 따라 다양한 관계 검정을 고른다.
- **다음으로 이어지는 점**: 11차시에서는 이런 관계 이해를 바탕으로 예측 모델 학습으로 넘어간다.
