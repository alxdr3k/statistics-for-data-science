...
| [점 추정](#1-점-추정과-구간-추정) | 모수를 하나의 숫자로 추정하는 방식 |
| [구간 추정](#1-점-추정과-구간-추정) | 모수가 있을 만한 범위를 제시하는 방식 |
...
- [점 추정](#1-점-추정과-구간-추정): 모수를 하나의 숫자로 추정하는 방식
- [구간 추정](#1-점-추정과-구간-추정): 모수가 있을 만한 범위를 제시하는 방식
- [최대우도법](#3-적률법과-최대우도법): 관측 데이터가 가장 그럴듯해지는 모수를 고르는 방법
- [신뢰구간](#4-대표-신뢰구간): 표본의 흔들림을 반영해 모수가 있을 법한 범위를 제시하는 방법
...
- **이전에서 이어지는 점**: 6차시가 주장 하나를 버릴지 판단했다면, 7차시는 모수가 있을 법한 범위를 말한다.
- **다음으로 이어지는 점**: 8차시에서는 숨은 변수나 복잡한 분포 때문에 직접 계산이 어려운 추정으로 넘어간다.
