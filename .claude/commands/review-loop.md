...
Path A 실행 중 `subscribe_pr_activity`/`unsubscribe_pr_activity` 호출이나 probe용 `gh api` 호출이 auth/scope/네트워크 등 진행을 막는 오류로 실패하면 즉시 Path B로 fallback한다. transient 오류는 같은 wake-up에서 1회 재시도해도 되지만, 영구 오류는 지체 없이 Path B 스크립트를 harness-tracked `Bash(run_in_background)`로 발사해(불가 시 foreground) polling-based 흐름으로 계속한다.
...
1. PR을 식별한다. 우선순위는 PR URL, PR 번호+명시 repo (`owner/repo`), 현재 cwd가 그 PR repo의 작업 branch일 때의 현재 브랜치 자동 감지 순서다. 현재 세션 cwd가 PR repo라는 보장이 없는 cross-cwd 상황에서는 인자 없음/PR 번호 단독 fallback을 쓰지 말고 PR URL 또는 PR 번호+repo를 요구한다. cwd==PR repo fallback일 때만 `gh pr view --json number,url,baseRepository,headRefName,headRepository,headRefOid -q .`로 현재 브랜치 PR을 찾는다. 감지 실패 시 사용자에게 PR 번호/URL을 요청한다.
   PR repo가 확정된 뒤의 `gh pr view`/comment/reaction 조회는 그 repo를 명시한다 (`gh -R <repo>` 또는 `gh api repos/<repo>/...`). cwd의 origin은 cross-cwd에서 source of truth가 아니다.
2. baseline timestamp + **reviewed_head_sha**를 한 번에 캡처한다. baseline 우선순위: ① Events API PushEvent (`repos/<head_repo>/events`의 `refs/heads/<branch>` push 중 최신 `created_at`) → ② PR timeline (`repos/<owner>/<repo>/issues/<pr>/timeline`의 `committed`/`head_ref_force_pushed`) → ③ HEAD 커밋 `committer.date`. 미래 시각은 현재로 클램프한다. 이후 wake-up에서 baseline 이전 활동은 무시한다. **이 baseline을 `last_push_at`으로도 기억해 둔다 — silent-approval probe의 기준 시각이 된다.** 동시에 `gh -R <repo> pr view <PR> --json headRefOid -q .headRefOid`로 현재 head SHA를 `reviewed_head_sha`에 기록한다. 이 SHA가 codex 리뷰가 평가하는 commit이며, Pass 신호 인계 시 payload에 들어가는 head_sha다.
...
1. PR metadata에서 PR head repo nwo (`headRepository`의 `owner/repo`)와 head branch (`headRefName`)를 확정한다. review-request 게시/대기용 repo(base repo)와 feedback mutation용 head repo가 다를 수 있으므로 cwd의 origin으로 대체하지 않는다.
2. feedback mutation worktree를 `resolve-pr-worktree.sh`로 해석한다. helper 탐색은 기존 deployed/source helper 패턴을 따른다:

   ```bash
   RESOLVE_PR_WORKTREE_HELPER=""
   for candidate in scripts/resolve-pr-worktree.sh .agents/scripts/resolve-pr-worktree.sh "$HOME/.agents/scripts/resolve-pr-worktree.sh"; do
     if [ -x "$candidate" ]; then
       RESOLVE_PR_WORKTREE_HELPER="$(cd "$(dirname "$candidate")" && pwd)/$(basename "$candidate")"
       break
     fi
   done
   [ -n "$RESOLVE_PR_WORKTREE_HELPER" ] || { echo "resolve-pr-worktree.sh 헬퍼를 찾지 못함" >&2; exit 1; }
   PR_WORKTREE="$(bash "$RESOLVE_PR_WORKTREE_HELPER" "$PR_HEAD_REPO" "$PR_HEAD_BRANCH" "$reviewed_head_sha")" || exit $?
   ```

   helper가 실패하면 feedback 수정을 진행하지 말고, stderr의 안내대로 PR head branch를 로컬 worktree로 먼저 checkout하거나 `reviewed_head_sha`와 일치하도록 update하라고 보고한다.
3. 수정, 검증, commit, push는 현재 세션 cwd가 아니라 `PR_WORKTREE`에서 수행한다. shell 명령은 `git -C "$PR_WORKTREE" ...` 형식으로 실행하고, 파일 편집/테스트 tool도 workdir을 `PR_WORKTREE`로 지정한다. 예: `git -C "$PR_WORKTREE" status`, `git -C "$PR_WORKTREE" add ...`, `git -C "$PR_WORKTREE" commit ...`, `git -C "$PR_WORKTREE" push origin "$PR_HEAD_BRANCH"`. cwd==PR worktree인 기존 흐름에서도 helper가 같은 worktree를 반환하므로 동작은 유지된다.
4. `last_push_at`을 새 push timestamp로 갱신한다 (baseline 재계산: 위 "진입" 2단계와 동일). **`reviewed_head_sha`도 새 push 후 head SHA로 갱신한다** (`gh -R <repo> pr view <PR> --json headRefOid -q .headRefOid`). 이후 review 사이클은 이 새 SHA를 기준으로 평가된다.
5. **review-request 재게시** (entry 3단계와 동일): 새 baseline 기준으로 `$CODEX_REVIEW_REQUEST_BODY` 코멘트를 1회 무조건 남긴다 (eyes 체크 없이). 자동으로 codex 리뷰가 트리거되지 않는 repo에서는 이 재요청이 없으면 무한 대기에 빠지고, 트리거되는 repo에서도 codex가 reaction-only로 끝낼 가능성을 막기 위해 무조건 게시한다. 이 게시는 동일 baseline 1회 한정.
6. subscribe는 이미 active이므로 재호출하지 않는다 (idempotent이지만 불필요).
7. turn 종료.
...
각 대기 사이클은 `wait-codex-review.sh`를 **harness-tracked `Bash(run_in_background)`로 1회 발사**하고, 완료 시 도착하는 completion notification의 exit code로 분기한다 (DEC-050). 스크립트는 self-terminating이라 내부 polling + 종료 시 결과 반환을 담당하므로, harness가 실제 프로세스 종료를 추적해 세션을 자동 재호출하며 exit code/stdout를 보존한다 — foreground로 묶어 turn을 막거나 Bash 도구 wall-clock 한도에 잘릴 이유가 없다. webhook(Path A)·harness-bg 둘 다 못 쓰는 환경에서만 foreground 1회 실행으로 대체한다. GitHub app으로 즉시 확인 가능한 상태가 있어도 대기/polling은 스크립트에 맡긴다. feedback을 수정하고 push한 뒤에는 다음 대기 사이클로 스크립트를 다시 발사한다.
...
기본 stdout은 사람이 읽는 feedback 출력이다. 구조화된 관찰이 필요하면 같은 호출에 `--json`을 붙이거나 `CODEX_REVIEW_OUTPUT=json`을 설정한다. 이 모드는 exit code를 바꾸지 않고 stdout에 compact `schema_version:1`, `kind:"codex_review_observation"` JSON 1개를 출력한다. (`Bash(run_in_background)` 발사 시 이 출력은 completion notification의 output 파일에 담겨 온다.)

다음 패턴은 금지한다. (harness-tracked `Bash(run_in_background)` 발사와 구분할 것 — 아래는 *raw 셸 백그라운드 + 수동 폴링*을 금지하는 것이지 harness-bg 발사 자체를 금지하는 게 아니다. harness-bg는 harness가 종료를 추적·자동통지하므로 허용된다.)

- raw `bash ... &` 로 harness 추적 밖 background polling
- background 실행 후 주기적으로 output을 수동 확인 (harness completion notification을 기다리지 않고)
- 매 sleep 사이에 PR 상태를 다시 polling
- 별도 monitor 도구로 stream watch

### 절차

1. PR 만든 직후, 또는 push 직후, 스크립트를 harness-tracked `Bash(run_in_background)`로 1회 발사한다 (harness-bg 불가 환경에서만 foreground).
2. 완료 notification을 기다린다 (foreground 대체 시엔 종료까지 대기). 스크립트가 PR 감지, baseline 계산, feedback/reaction polling을 처리한다.
3. 종료 코드에 따라 처리한다.
...
- push 후:
  ...
  - **Path B**: 스크립트를 harness-bg로 다시 발사한다 (불가 시 foreground).
...
Pass 신호 감지 시 payload emit **직전**에 PR live head를 다시 fetch한다 (`gh -R <repo> pr view <PR> --json headRefOid -q .headRefOid` → `live_head_sha`).
...
- 인자 없음: 현재 cwd가 PR repo의 작업 branch일 때만 현재 브랜치 PR 자동 감지
...
cross-cwd 상황에서는 PR URL을 선호한다. PR 번호만 전달하려면 호출자가 repo(`owner/repo`, 예: `CODEX_REPO`)도 함께 명시해 helper가 현재 cwd의 origin에 의존하지 않게 한다.
