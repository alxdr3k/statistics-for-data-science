...
- `--opus-review`: Review Pass를 항상 Opus sub-agent로 실행한다. Step 9에서 `/review-and-land`를 실행할 때도 `--opus-review`를 전달한다.
...
- PR polling, pass reaction 확인, comment fetch 같은 상태 확인은 `review-and-land`/helper script에 맡긴다. LLM이 필요한 요약/분류가 있을 때만 Haiku 또는 read-only Explore를 쓴다.
...
`/codex:review`·`/codex:adversarial-review` 서브커맨드는 `--model` 플래그를 받지 않는다 (companion 1.0.4 기준 `--model`은 `task` 서브커맨드 전용). `adversarial-review`에 넘긴 positional 인자는 review focus text로 처리되고, `review`는 focus text가 있으면 거부한다(throw). 따라서 `--model gpt-5.5` 같은 인자를 붙이면 모델 고정이 아니라 focus 오염(adversarial-review) 또는 리뷰 실패(review)가 된다. reviewer 모델은 codex 기본값/`~/.codex/config.toml`에서 결정하며, run은 인자로 고정하지 않는다.

리뷰어 호출 인자 표면(주의): (i) worktree 흐름에서 diff가 task worktree에 있으면 codex review에 `--cwd <worktree>`를 명시해야 diff를 본다 — main 세션 cwd(clean)에서 돌리면 빈 diff를 리뷰한다. (ii) native `review`(defect pass)는 startup에 git object probe가 있어 history가 큰 repo에서 수십 초~분 느릴 수 있다(adversarial-review엔 없음); 진행 정체로 오인하지 말고 출력 파일 byte 증가·pid 생존으로 교차 확인한다. (iii) codex 호출 stdin은 단일 소스만 준다 — prompt를 `- < promptfile`로 줄 때 `</dev/null`을 함께 주면 빈 프롬프트로 헛돈다.
...
- (iii) **executable agent contract를 하나도 바꾸지 않는다** — diff에 `commands/**`·`codex/skills/**`·`scripts/**`·`deploy.sh`·`tests/**`·`AGENTS*.md`·`CLAUDE.md`가 있으면 design-doc acceptance **불가**다. 이들은 실행 계약이라 강한 "0 actionable" 리뷰 대상이고, dense spec이 아니라 codex 리뷰가 수렴한다(이 PR(PA-4.3)처럼 `commands/run.md`·`codex/skills/run/SKILL.md`를 바꾸는 슬라이스는 design-doc 슬라이스가 **아니다**).
...
3. **종료 판정** — 슬라이스 종류로 분기:
   ...
   - **design-doc/spec 슬라이스 (위 게이트 통과 시에만)**: 이번 pass에 (a) architecture 반증(high·approach-overturn) **0** AND (b) 문서 내부 모순 **0**이면 그 round를 **clean**으로 카운트. **2회 연속 clean**이고 잔여 precision finding이 모두 impl-scoped 항목(이슈/슬라이스)으로 **등록 완료**됐으면 Step 7로 간다 (**design-doc acceptance pass**). 조건 미충족이면 4로. architecture 반증·내부 모순이 남아 있으면 절대 clean이 아니다(consistency/precision edge만 남았을 때 비로소 clean).
...
- 검증을 통과하면 `/review-and-land "$PR_NUMBER"`를 실행한다. `--opus-review`가 있으면 `/review-and-land --opus-review "$PR_NUMBER"`로 실행한다.
...
- `/review-and-land`이 review feedback 처리, checks 확인, merge까지 완료해야 한다. 해당 PR이 merge되기 전에는 cycle을 마치거나 다음 loop로 넘어가지 않는다.
