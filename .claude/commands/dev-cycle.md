...
- 의도한 파일만 stage, commit, `DEV_CYCLE_WORK_BRANCH` push. 이어서 PR body를 `PR_BODY_FILE` 경로의 파일로 작성하고 (아래 'Test plan 섹션' 참고) `check-test-plan`을 통과한 뒤 `gh pr create`로 **draft가 아닌 open PR**을 생성한다. 생성 결과 URL에서 PR 번호를 추출해 `PR_NUMBER`에 저장하고 Step 9에서 같은 변수를 사용한다.
